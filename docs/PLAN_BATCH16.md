...
### 72cm. **找到了 otps 的真正缺口：多行不摊薄**（第 151 轮）

Splash 的 144/170 tok/s 是**并发聚合**数字，所以我第一次量了并发。结果：

| 并发 | wall | 聚合 otps | 每流 |
|---|---|---|---|
| 1 | 9.45 s | 13.55 | 13.55 |
| 2 | 20.59 s | 12.43 | 6.22 |
| 4 | 30.46 s | **16.81** | 4.20 |

**聚合几乎不涨**（13.55 → 16.81，只有 1.24×），而 Splash 在 4 并发上是 **3.9×**。

**调度器是好的**：`QW_STEP_TIME` 显示 n=4 时是 **127 次 pass、每次 4 行**
（127×4=508≈512 token），也就是 4 个 slot 确实被合成一个 pass。
...
**4 行花了 3.44 倍时间**，而不是接近 1 倍。
（单流 38.4 ms = 26 tok/s，相对 481 GB/s / 14.4 GB 的 30 ms 上限是 78%，这一头其实不差。）

**⇒ 权重扫描被按行重复了。** 4 行本该和 1 行读同样多的权重（NRB=256，4 行也只占 1 个 token block），
所以 4 行 pass 应该≈38-45 ms。修好它，4 并发聚合≈**4×26 = 104 tok/s**，
而 Splash 的 170 tok/s 是在 **16 核 GPU 的 M5 Pro** 上测的，我们这台是 40 核 M5 Max。

**`QW_GEMM_MIN_ROWS=4` 没有改善**（16.60 vs 16.81），所以不只是「行数低于阈值退回标量路径」
那一条 —— `gemm_min_out_f()` 里还有 `rows >= 32` 的条件（linear.rs:519），
MPP 通路本身在多行上可能也是线性放大的。**这是下一轮的第一目标，也是整个项目里价值最高的一处。**

### 72cn. 更正 72cm：**GEMM 摊薄行数没问题，缺口在「每个序列」的那部分**（第 152 轮）

上一轮我下结论说「权重扫描被按行重复了」。**这个结论错了。**

**证据一：`gemm-bench` 的时间对行数完全平。**
...
1 行到 16 行，497 个 linear 的总时间**一动不动**（0.46-0.48 s）。
（注：这个绝对值本身偏低效——0.4672 s 扫 14.4 GB 只有 31 GB/s，
说明 `gemm-bench` 在小 token 数下被固定开销主导，只能用来读**斜率**，不能读绝对值。）

**证据二：同样的 4 行，来自 1 个序列 vs 来自 4 个序列，差一倍。**

| pass | 中位 |
|---|---|
| 4 行 / **4 个 slot** | **132.2 ms** |
| 4 行 / **1 个序列**（prefill 尾块） | **66.9 ms**（本轮，机器较吵）；早先安静时 38.9-46.9 ms |
| 1 行 / 1 个 slot | 38.4 ms |

**⇒ 行的摊薄是好的；多出来的开销来自「这 4 行属于 4 个不同的序列」。**
也就是 attention / GDN 的 state 与卷积窗口 / KV append 这些**每序列**的工作没有跨 slot 摊薄。

这正好解释了并发聚合为什么是平的（13.55 / 12.43 / **16.81** @ 1/2/4）：
每次 pass 的权重扫描没变，但每多一个 slot 就多付一份**每序列开销**，
于是 4 并发的收益被吃掉了。Splash 在这一点上是 3.9×。

**下一轮的目标：把每序列的非 GEMM 工作跨 slot 批处理。**
...
### 72cs. TILE=6 试验：**无法定论**，机器噪声把 A/B 毁了（第 157 轮）

**动机**：`TILE` 决定一次 verify 能结算几个 token（`TILE-1` 个 draft）。
`QW_SPEC_DEBUG` 显示接受数经常打满窗口（`out=4`），所以想试 `TILE=6`
（代码里已经有 `K_Q4_GEMV_K6_U4H`，`match TILE { 6 => ... }`）。

**短跑（128 token）看着像赢**：TILE=6 中位 37.26，TILE=4 中位 34.80。
但两组区间重叠（TILE=4 是 32.31-39.00），**不显著**。

**改用 512 token（prefill 摊薄，更稳）后：**

| 构建 | 三次 otps | 中位 |
|---|---|---|
| TILE=4 | 45.19 / 43.04 / 39.18 | **43.04** |
| TILE=6 | 34.15 / 23.74 / 23.55 | 23.74 |

看着是 TILE=6 **倒退 1.8 倍**。**但随即把 TILE 改回 4 重建、同一个二进制再跑，
变成 26.72 / 16.16** —— 而几分钟前同一个 TILE=4 二进制是 45.19/43.04/39.18。

**⇒ 机器负载在实验中途漂了（墙钟 8.2-9.9 s → 14.4-23.9 s），
TILE=6 那三次很可能正好落在吵的窗口里。这个 A/B 无效，两边都不能下结论。**

**处理**：`TILE` 保持 4（即 HEAD 原值），工作树干净（`git diff` 为空），
**没有把任何未验证的改动留成默认**。

**教训**：`accept.sh` 能证明**正确性**，证明不了**性能**。
性能 A/B 必须**交错**测（A/B/A/B）或用成对比较，否则机器漂移会伪造出
1.8 倍的假结论。**当前机器噪声已经大到无法验证任何性能改动** ——
这是继续优化前的硬阻塞。

### 72ct. TILE=6 是**已验证的倒退**；并且交错 A/B 的方法学成立了（第 158 轮）

机器安静后（同一二进制连跑三次 44.03/40.21/41.75，±5%，之前是 ±40%），
把两个构建都编出来交错测：

```
/tmp/q4  md5 3688d6da1793d7b0879aa59fb1c91e92   (TILE=4)
/tmp/q6  md5 172b6e75fa266f07ee0a90341d7dc7f5   (TILE=6)
```
...
**TILE=4 三对全胜。** 而且注意两列是**一起**往下漂的
（41.51→33.03→23.58 和 32.64→22.77→20.32）—— 漂移是真的，
**但比值稳定在 1.16-1.45，而绝对值掉了一半。配对把漂移消掉了。**

**这正是上一轮那个无效 A/B 缺的东西**：不配对时，45.19 对 23.74 看起来像
1.8 倍倒退；配对后真实代价是 **16%-45%**。

**结论**：`TILE` 保持 4。TILE=6 虽然多 draft 两个 token（5 vs 3），
但一次 verify 处理 6 行的代价（快照/`commit_row` 拷贝、attention 6 个 query 行）
超过了多接受的那点收益。**不要往这个方向再投入。**
...
### 72cz. **GEMM 不是瓶颈**：孤立扫描跑到 465 GB/s（97%），缺口在那 689 个非 GEMM 小 dispatch（第 164 轮）

上一轮算出「1 行 pass 只跑到 62% 带宽」，于是怀疑 kernel 内部访存。**用 `bench --rows`
扫了一圈，结论完全相反：**

| rows | ms/token | effective GB/s |
|---|---|---|
| 0 | 32.700 | 440.7 |
| 1 | **31.047** | **464.2** |
| 2 | 31.985 | 450.6 |
...
**孤立的 497 个 linear 扫描是 31.0 ms = 464 GB/s = 481 的 97%。GEMV 本身几乎打满带宽，
而且对 rows 完全不敏感（0/1/2/4 全在 31-32 ms）。**

**真正的 1 行 pass 是 48.6 ms。差额：**

```
线性扫描（497 个 GEMM）  : 31.0 ms   = 464 GB/s（97%）
真实 1 行 pass           : 48.6 ms   = 297 GB/s
非 GEMM 部分             : 17.6 ms   = 整个 pass 的 36%
```

**689 个非 GEMM dispatch 分摊这 17.6 ms ⇒ 每个约 25 µs。**

而 GDN 的 state + 卷积窗口流量总共才约 1.16 GB ≈ 2.4 ms。
**⇒ 这 17.6 ms 不是流量，是「689 个小 kernel 的启动/延迟开销」。**

**这同时解释了 4 行为什么贵**：4 行 pass 的 GEMM 仍是约 31 ms（rows 免费），
实测 61-131 ms，所以非 GEMM 部分在 4 行时是 30-100 ms ——
**GEMM 对行数免疫，而这一堆小 kernel 随行数增长。这就是行摊薄失败的真正位置。**

**方向修正（重要）：** 不是 kernel 内部访存、不是路由、不是调度，
...
### 72dc. 接受率 **1.93/3**；单流稳态其实是 **57 tok/s**，不是 40.5（第 167 轮）

**接受率分布**（44 个 spec step，短 prompt）：
...
**平均接受 1.93 个 draft（3 个里），48% 的 step 三个全中。
加上已结算的那个 token，每个 step 产出 2.93 个 token。**
（这也解释了为什么 TILE=6 反而更慢：verify 占 85%，多要两行不划算。）

**关键更正：速率要用均值，不能用中位数。**

```
MEAN   draft  7.8 ms | verify 43.4 ms | step 51.2 ms
MEAN tokens/step 2.93 -> 57.3 tok/s
```

而实测 harness 只有 40.5 tok/s。**差额在墙钟里：**

```
spec step 合计 2.251 s (draft 0.341 + verify 1.910)，墙钟 3.16 s
-> 0.91 s 在 spec step 之外（prefill + 首请求开销）
-> 稳态 128/2.251 = 56.9 tok/s
```

**⇒ 单流稳态是 57 tok/s，不是 40.5。对 Splash 的 74 是 77%，而不是 55%。**

verify 43.4 ms 对 31.0 ms 带宽下限是 **1.4 倍** —— 这是单流唯一剩下的余量。

**方法学教训（第二次犯同类错）**：吞吐是「总量 ÷ 总时间」，
**中位数会把长尾藏起来**。第 158 轮定的规矩要补一条：
速率类指标一律用**均值**，或者直接量总时间。

### 72dd. 聚合稳态是 **79.4 tok/s**（Splash 的 47%），不是 27.2；4 行 pass = 1.60 倍 1 行（第 168 轮）

沿用上一轮的更正（速率要用总量÷总时间），把 n=4 重测一遍。**n=4 时投机是关的，
所以 pass 会被 `QW_STEP_TIME` 正常记录**，可以直接算稳态：

```
n=4: 512 tokens, 墙钟 8.71 s, harness 报 58.76 tok/s
  127 个 4 行 pass, MEAN 50.8 ms, 合计 6.45 s
  -> 2.26 s 在 pass 之外（4 次 prefill + 开销）
  -> 稳态 512/6.45 = 79.4 tok/s
```

**⇒ 聚合稳态 79.4 tok/s，不是之前报的 27.2（那个是噪声窗口的读数）。**

```
4 行 pass 50.8 ms vs 1 行 31.7 ms = 1.60 倍
若压到 31.0 ms 带宽下限 -> 512/(127*0.031) = 130 tok/s
```
...
**⇒ 两个指标都比我一直报的好得多，而且缺口的形状清楚了：
4 行 pass 的 1.60 倍就是全部余量。把它压到 1.0 倍，聚合就是 130 tok/s。**

**注意 harness 报的数字（58.76）包含 2.26 s 的非 pass 时间（4 次 prefill + 首请求开销），
所以对外报数要说明是「稳态」还是「端到端」——两者差 35%。**

### 72de. 行摊薄的形状：**第 2 行要 12.5 ms，第 3、4 行只要 3-4 ms**（第 169 轮）

干净的扩展曲线（`QW_SPEC=0`，均值）：

| rows | 平均 pass | 聚合 tok/s | 边际成本 |
|---|---|---|---|
| 1 | 31.7 ms | 31.5 | — |
| 2 | 44.2 ms | 45.2 | **+12.5 ms** |
| 3 | 48.1 ms | 62.4 | +3.9 ms |
| 4 | 50.8 ms | 78.8 | +2.7 ms |

**⇒ 边际成本极不均匀：第 2 行要 12.5 ms，之后每行只要 3-4 ms。**

1 行 pass 是 31.7 ms，与孤立权重扫描的 31.0 ms 几乎相同（**94.5% 带宽**）。
所以 **31.7 → 44.2 这 12.5 ms 不是「多算一行」的钱，而是「换了一个 kernel」的钱**：

* `rows == 1` → `q4_gemv_hx`（单 token 核）
* `rows >= 2` → `encode_tile` / `K4_U4HX`（四行特化核）

而 `K4_U4HX` 的注释本身就写着「fed a single row it still computes four」——
它是为 4 行设计的，但**每次权重扫描的带宽效率低于单 token 核**。

**⇒ 剩下要拿的就是这 12.5 ms + 后续的小尾巴。**
把多行 tile 核的带宽效率提到单 token 核的水平，4 行 pass 就能从 50.8 ms 降到约 33 ms，
聚合从 **78.8 → 约 121 tok/s**（Splash 的 71%），单流也会同步受益。

**注意**：这里已经排除过的方向——GEMM 路由（慢 5-10 倍）、TILE=6（`K6_U4H` 更慢）、
encoder split（免费）、CPU/派发（免费）、occupancy/合并访存等参数调整（早先系统性证伪）。
**剩下的是 `K4_U4HX` 本身的访存效率**，这是一个具体、可测的目标。

### 72df. **多行 GEMM 不是「行免疫」**：4 行扫描只跑到 191 GB/s（第 170 轮）

上一轮推断「12.5 ms 是换 kernel 的钱，非 GEMM 在 1 行时很小」。
**用 `bench --tokens` 直接扫 497 个 linear 的孤立成本，结论要再改一次：**
...
（`ms_per_token` × tokens = 每次扫描；1 行 27.0 ms 甚至超过 481 GB/s 的参考值，
说明单行核的效率已经很高。）

**边际成本：1→2 +19.4 ms，2→3 +15.1 ms，3→4 +14.0 ms —— 近乎线性。**

**⇒ 多行 GEMM **不是**行免疫的。它的带宽效率从 534 GB/s 掉到 191 GB/s（36%）。
「行摊薄失败」的位置就在多行 tile 核本身。**

**这一条推翻了前两轮的两个中间结论**（「GEMM 对行数免疫」和「缺口在非 GEMM」），
但把目标钉得更准了：**`K4_U4HX` 在多行时的带宽利用率只有单行核的 36%。**

**注意口径差异**：孤立 bench 的 4 行扫描是 75.5 ms，真实 4 行 pass 是 50.8 ms，
两者配置不同（bench 用 `--rows 0` 的 round-3 核），所以只能读**趋势**，不能读绝对值 ——
这与第 158 轮定的规矩一致。

**下一步（具体）**：读 `K4_U4HX` 的 MSL，找出为什么 4 行时每线程的访存效率下降。
...
### 72di. n=4 的墙钟账（修正 2.26 s 那笔）（第 173 轮）

上一轮说「2.26 s 在 pass 之外」，那是用「4 行 pass 合计」直接减墙钟得出的，
**漏算了 prefill 本身也是 pass**。完整的账：

```
n=4 短 prompt，墙钟 8.71 s
  4 行 decode pass 合计 : 6.45 s (74%)  -> 79.4 tok/s
  prefill               : 1.16 s (13%)  -> 164 行 + 16 行，两个 pass
  所有 pass 之外        : 1.10 s (13%)
  含 prefill            : 512/7.61 = 67.3 tok/s
  端到端                : 512/8.71 = 58.8 tok/s
```

**⇒ prefill 已经是跨请求融合的（一个 164 行的 pass），
所以「prefill 没并发」这个判断是错的——它已经是批处理的。**

剩下的 1.10 s（13%）是请求级开销（首请求、tokenize、模板、SSE 等），
不是 kernel 问题，也不是 prefill 问题。

**修正后的口径表（对外报数必须选一个并说明）：**

| 口径 | 聚合 otps | 说明 |
|---|---|---|
| 纯 decode | 79.4 | 只算 4 行 pass |
| 含 prefill | 67.3 | decode + prefill |
| 端到端 | 58.8 | 含全部请求开销 |

**结论：prefill 不是缺口（已融合），请求开销 13% 也不是主要矛盾。
74% 的时间在 decode，而 decode 的每 pass 效率已经由第 172 轮证明
没有可用的 kernel 杠杆了。**

### 72dj. 「多 slot 共享一次 verify」**不可行**：8 行 pass 要 361 ms（4 行的 7 倍）（第 174 轮）

我上一轮提出要评估「多 slot 共享一次 draft/verify」。
**在动手之前先量了行扩展的远端，结果直接否掉了这个方案。**

| rows | 平均 pass | ms/行 | 聚合 tok/s |
|---|---|---|---|
| 4 | **50.8 ms** | 12.7 | **78.8** |
| 8 | **361.2 ms** | 45.2 | 22.1 |
| 16 | 384.9 ms | 24.1 | 41.6 |

**⇒ 4 行到 8 行之间有断崖：8 行 pass 是 4 行的 7.1 倍。**

原因是 `forward_rows` 在 `rows > TILE(4)` 时走**另一条路径**（网格映射核，
`encode_b`「grid grows with the batch」），而不是 `encode_tile` 的四行特化核。

**⇒ 设想的「4 个 slot 各 draft ~4 个 token、一次 16 行 verify 结算 ~16 个 token」
会是 16 token / 385 ms = 41.6 tok/s —— 比现在 4 行 78.8 差一倍。**

**方案作废。而且这是量出来的，不是推出来的。**

**修正我在第 168 轮（§72cw）的判断**：当时说「n=4 已经每次 pass 拿 4 个 token，
投机在 n=4 上加不了东西」——**这个结论本身是对的，但我给的理由不完整**。
真正的理由是：**4 行是这条架构的甜点，再宽就掉进网格映射路径的断崖。**
所以 n=4 的 4 个 token/pass 已经是该架构下的上限。

**⇒ 聚合 78.8 tok/s（Splash 的 46%）在这套架构里没有可用的杠杆了。**
要再往上，需要的是**换掉 rows>4 的网格映射路径**（让宽 pass 也保持 4 行特化核的效率），
那是一次 kernel 重写，而不是参数或路由调整。

### 72dk. 用实测接受率反推 TILE：**TILE=4 是数学最优，且算术复现了实测 57.2 tok/s**（第 175 轮）

有了接受率分布（0/3×7、1/3×10、2/3×6、3/3×21，共 44 个 step），
就可以对 TILE 做纯算术评估。生存函数：

```
P(接受>=1)=0.841  P(>=2)=0.614  P(>=3)=0.477  平均接受 1.932
```

每次 pass 的 token 数 = 1（已结算的）+ 累计接受概率。
verify 成本按行插值（1 行 31.7、4 行 43.4 ms），draft 每步 7.8/3 = 2.6 ms：

| TILE | tokens/pass | verify | step | tok/s |
|---|---|---|---|---|
| 2 | 1.84 | 38.0 ms | 40.6 ms | 45.3 |
| 3 | 2.46 | 41.0 ms | 46.2 ms | 53.1 |
| **4** | **2.93** | **43.4 ms** | **51.2 ms** | **57.2** |

**⇒ TILE=4 是最优，且这一行算术复现了实测的 57.2 tok/s —— 模型和测量吻合。**

**这解释了为什么 TILE=6 会输**（第 157 轮实测 1.16-1.45 倍更慢）：
多 draft 一步换来的接受率增益，抵不过 verify 多两行的成本。
**TILE 的问题到此结束，不需要再试。**

**同时也说明：单流 57 tok/s 这个数不是「哪里出了错」，而是
「TILE=4 + 1.93 接受率」这个配置的数学结果。要更快只有两条路：
verify 从 43.4 降到 31.8（→74 tok/s，即 Splash 水平），或提高接受率。**

### 72dl. 给 U4 家族补一个 NK=4 成员：**模型里更慢，已还原**（第 176 轮）

上一轮的算术说明：单流要达标只需 verify 从 43.4 → 31.8 ms。
而 `U4` 家族（bench 里 472 GB/s）**只有 NK=3 成员**，所以补了一个 NK=4：

读宏定义发现两族的关键差异：

```
#define Q4_GEMV_KS_U4(NAME, NK)    ... (void)R;   // 用 k：一个 threadgroup 管一行输出、k 个 token
#define Q4_GEMV_KS_U4H(NAME, NK)   ... (void)k;   // 用 R：一个 threadgroup 管 R 行输出
```

**⇒ `U4` 的网格是 `out_f` 个 threadgroup（满并行，和 `q4_gemv_hx` 一样），
而 `U4HX` 是 `out_f/R`（并行度降 R 倍）。** 这正是我想补 NK=4 的理由。

于是加了 `Q4_GEMV_KS_U4(q4_gemv_k4_u4, 4)`，把 `TILE=4` 指过去：

**正确性通过**（`fastchk` md5 `5b5f6e931dddd4cc943589f4380e2325`，与基准完全一致）。

**但真实 4 行 pass 的交错 A/B（3 对）说它更慢：**

| 变体 | rep1 | rep2 | rep3 | 均值 | 带宽 |
|---|---|---|---|---|---|
| `U4HX`（现状） | 51.6 | 51.5 | 52.9 | **52.0 ms** | 277 GB/s |
| `K4_U4`（新） | 56.4 | 56.8 | 78.2 | 63.8 ms | 226 GB/s |

**⇒ 慢 23%，已还原（工作树干净）。**

**结论（第二次验证同一教训）**：`bench` 的隔离口径**不能预测模型里的行为**。
原因和 §72dh 里 `encode_tile` 注释写的一样——bench 让 497 个 linear 共用同一个 x 缓冲区，
x 常驻 cache，**任何「省 x 流量 / 提高 x 复用」的优化在 bench 里都被系统性高估**。

**⇒ 单流 verify 的 11.6 ms 缺口，不能用「换 U4 家族」拿到。
`U4HX`（每 threadgroup R 行、x 常驻寄存器）已经是模型里最快的四行核。**

### 72dm. 收尾前的状态复核（第 177 轮）

在干净的 `HEAD` 上重新量了一遍四项指标：

| 指标 | 本次 | 备注 |
|---|---|---|
| 冷 TTFT（~2000 字符 prompt） | **1749 ms** | 服务 ready 后第一个请求 |
| 暖 TTFT | **34 / 19 ms** | 同一 prompt 的第 2、3 次 |
| 单流 otps（稳态） | **57.3** | TILE=4 + 1.93 接受率的算术结果，已复现 |
| 聚合 otps（n=4 稳态） | **79.4** | 纯 decode；含 prefill 67.3、端到端 58.8 |

**注意口径**：这里的「冷」是**服务 ready 之后的第一个请求**，不含模型加载。
与 llama.cpp / Ollama 的对比来自早先的配对测量（冷约差 10%，暖约赢 8 倍），
**本轮没有重跑对手**，所以那一行是历史数据，不是本轮实测。

**目标判定**：TTFT 一半达标；otps 一半未达标（单流 77%、聚合 46%）。
**所有 kernel 杠杆已证伪**（见 §72dh、§72dl 等），
剩下的是架构级改写（`rows>4` 网格映射路径），风险落在已经达标的冷 TTFT 上。

---

## 收尾总结（第 178 轮，用户决定以当前成果交付）

### 达成的

| 指标 | 结果 | 目标 |
|---|---|---|
| **暖 TTFT** | **19-34 ms** | ✅ 比 llama.cpp / Ollama 快约 8 倍 |
| **冷 TTFT** | **约差 10%**（配对测量） | ✅ 「差不多」达成 |
| **单流 otps** | **57.3 tok/s**（Splash 74 的 **77%**） | ⚠️ 接近 |
| **聚合 otps** | **79.4 tok/s**（Splash 170 的 **46%**） | ❌ 未达成 |

### 交付物

* **投机解码（DFlash 风格的 MTP draft + 4 行 verify）默认开启**，
  在任意单个 decoding slot 上生效（`QW_SPEC=0` 可关）。
* **正确性门禁全绿**：`accept.sh` 19 passed / 0 failed（含 mlx-lm oracle 对拍）、
  `gemm-check` 497 个张量过容差 0、`fastchk` md5 与基准逐字节一致。
* **口径可复现**：`QW_SPEC_DEBUG` 打印 draft/verify 拆分与接受率；
  `QW_STEP_TIME` 打印每次 pass 的行数与耗时。

### 关键数据（供后来者）

* 单流 57.3 tok/s 是 **TILE=4 + 1.93 接受率**的**算术结果**（算术复现了实测值），
  不是缺陷。要更快只有两条路：verify 43.4 → 31.8 ms，或提高接受率。
* 4 行 pass 是这套架构的**甜点**；8 行要 361 ms（7.1 倍），
  因为 `rows > TILE` 走的是网格映射路径。
* 实测接受率分布（44 个 step）：0/3 16%、1/3 23%、2/3 14%、**3/3 48%**。

### 已证伪的方向（均为配对测量，不要重走）

| 方向 | 结果 |
|---|---|
| 参数级调优（occupancy / 合并访存 / 指令数 / 预取 / M-N tile / KT / relaxed） | 无收益 |
| 行阻塞（`bench --rows`） | 端到端慢约 5% |
| GEMM 路由（rows 2-4 走 MPP） | 慢 5-10 倍 |
| TILE=2 / 3 / 6 | 均更慢（算术与实测一致） |
| encoder split | 免费（无收益） |
| CPU / 派发开销 | 免费（GPU 99.1% 忙） |
| `U4H` 家族替换 `U4HX` | 无改善 |
| 给 `U4` 家族补 NK=4 成员 | 慢 23% |
| 多 slot 共享一次 verify（16 行） | 41.6 tok/s，比现状差一倍 |

### 两条方法学教训（各犯过两次）

1. **速率必须用总量 ÷ 总时间（均值），不能用中位数**——中位数会藏住长尾。
   用中位数时我报过 40.5 tok/s，实际稳态是 57.3。
2. **`bench` 的隔离口径不能预测模型行为**——它让 497 个 linear 共用同一个 x 缓冲区，
   x 常驻 cache，**任何「省 x 流量 / 提高 x 复用」的优化都被系统性高估**。
   两次实证：行阻塞（bench 快 2 倍，模型慢 5%）、NK=4 的 `U4` 核（bench 472 GB/s，模型慢 23%）。
   **任何 bench 收益都必须先在模型里做交错 A/B。**

### 未走的路

唯一剩下的是**重写 `rows > 4` 的网格映射路径**，让宽 pass 保持四行特化核的效率。
潜在收益大（聚合有望翻倍），但风险明确落在**已经达标的冷 TTFT** 上，用户选择不冒这个险。

### 交付验证（第 179 轮）

在**交付提交 `11ece7c` 本身**上重跑完整验收：
...
包含：投机 == 非投机（CLI 与 server 两侧）、server == CLI、
前缀缓存正确性、position pin、oracle 对拍（mlx-lm）、
prefill 分块宽度确定性（chunk 4 与 chunk 32 同答案）。

**⇒ 交付的这棵树是经过完整门禁验证的，不是「之前某一版验证过」。**

（`stock/` 下有用户自己的文件改动，与本项目无关，未触碰。）

### 72dn. **TTFT 对比重测：冷 TTFT 我之前的「差 10%」是错的，实际比 llama.cpp 慢 2.03 倍**（第 180 轮）

收尾后复核「TTFT 达标」这个声明——之前用的是**历史配对数据**（冷差约 10%、暖赢约 8 倍）。
在同一台机器、同一个 ~2000 字符 prompt 上**重新跑三个引擎**：

| 引擎 | 冷 TTFT | 暖 TTFT |
|---|---|---|
| **ours** | **1749 ms** | **34 / 19 ms** |
| llama.cpp（build 10809） | **863 ms** | 93 / 88 ms |
| Ollama（qwen38-27b:bench） | 2131 ms | 71 / 73 ms |

**⇒ 修正后的判断：**

* **暖 TTFT：达标且超出。** 比 llama.cpp 快 **2.6-4.6 倍**，比 Ollama 快 **2.1-3.7 倍**。
* **冷 TTFT：介于两者之间，但没有「和 llama.cpp 差不多」。**
  比 Ollama 快 1.22 倍（1749 vs 2131），但比 llama.cpp **慢 2.03 倍**（1749 vs 863）。

**⇒ 我之前写的「冷约差 10%」是错的**（那个数字来自不同的 prompt 规模/机器状态）。
**按同一口径实测，冷 TTFT 是 llama.cpp 的 2.03 倍。**

**对目标的含义**：用户的要求是「冷 TTFT 和热 TTFT 都做到和 llama.cpp 和 Ollama 差不多」。
* 暖 TTFT：**两个对手都赢**，明显达标。
* 冷 TTFT：**赢了 Ollama，输给 llama.cpp 2 倍**——按「和 llama.cpp 差不多」的严格口径，**这一项未达标**。
...
### 72dq. tail pass 预热**无效**——pipeline 编译假设被证伪（第 183 轮）

上轮假设 357 ms 空隙是 **4 行 tail pass 的 kernel pipeline 首次编译**。
理由：服务启动时的预热做的是 **64 行** pass，走**网格映射路径**，
**从不走 4 行 tile 路径**（`K4_U4HX`），所以那条 pipeline 会在客户端的第一个请求里现编译。

于是在启动预热里加了一次 **4 行 tail 形状**的 pass（和现有预热同一个位置、
同一套 `reset_seq` + `set_tokens` + `forward_rows`）。

**交错 A/B（3 对，冷 TTFT）：**

| 变体 | rep1 | rep2 | rep3 | 中位数 |
|---|---|---|---|---|
| 无 tail 预热（现状） | 1401 | 1393 | 1401 | **1401 ms** |
| 有 tail 预热 | 1539 | 1393 | 1397 | **1397 ms** |

**⇒ 没有改善。pipeline 编译假设被证伪，改动已还原（工作树干净）。**

**另一个值得记录的观察**：本次会话冷 TTFT 的实测值散布在
**1201 / 1236 / 1749 / 1393 / 1401 / 1397 / 1539 ms**，
**range 1201-1749 ms（±19%）**。所以：
1. 冷 TTFT 这个指标本身噪声很大，**任何小于 ~20% 的改动都无法用单次测量判定**；
2. 之前第 180 轮报的「1749 vs llama.cpp 863 = 2.03 倍」**落在噪声高端**，
   更稳健的说法是「约 1400 ms 对 863 ms，约 1.6 倍」。

**⇒ 357 ms 空隙的成因仍未定位。已排除：`logits_row`+argmax（0.5 ms）、
逐行采样（已修）、prefix 快照（已禁用）、tail pipeline 编译（本轮）。**

### 72dr. 空隙**随 prompt 行数线性增长**：每行约 0.85 ms（第 184 轮）

不再猜成因，改测**空隙是否随 prompt 规模变化**。三个不同长度的 prompt：

| prompt | prefill 行数 | prefill 耗时 | **空隙** | **空隙/行** |
|---|---|---|---|---|
| 100 字符 | 19 | 406.2 ms | **18 ms** | 0.95 |
| 400 字符 | 82 | 377.0 ms | **69 ms** | 0.84 |
| 1600 字符 | 349 | 737.7 ms | **293 ms** | 0.84 |

**⇒ 空隙不是固定编排开销，而是随行数线性增长，约 0.85 ms/行。**

**校验**：439 行 × 0.85 = **373 ms**，与实测的 357-375 ms 吻合。

**⇒ 这把范围缩小到 post-pass 区块里的一段「按 prompt 行」的工作。**

**注意对比**：prefill 本身的每行成本是随规模**下降**的
（21.4 → 4.6 → 2.1 ms/行，固定开销被摊薄），
**而空隙的每行成本是恒定的 0.85 ms** —— 说明它是纯线性的一段逐行工作。

**已排除的候选**（累计）：`logits_row`+argmax（0.5 ms）、逐行采样（已修）、
prefix 快照（28.6 ms 且已禁用）、tail pipeline 编译（本轮前的 A/B 证伪）。

**下一步**：在 post-pass 区块里找那段「按 prompt 行」的循环。
最可能是 prefix 缓存的 **ladder 记录**（每个 chunk 记一个边界）——
但 `QW_PREFIX_SNAPSHOT=0` 只关了快照，**ladder 可能仍在跑**。
验证方法：设 `QW_PREFIX_*` 相关开关重测空隙是否消失。

### 72ds. 空隙确认是 **0.85 ms/prompt 行**，与分块方式无关（第 185 轮）

用一个能区分「按行」和「按 pass」的实验：把 prefill 切成小块。
若空隙按 **pass** 计，则块数变多时总空隙应按块数增长；
若按**行**计，则总空隙只跟总行数有关。

| 配置 | pass 数 | 每 pass 空隙 | **每行空隙** |
|---|---|---|---|
| 默认 chunk=1020 | 1 × 439 行 | 357 ms | **0.81 ms** |
| `QW_PREFILL_CHUNK=64` | 6 × 64 行 | **55 ms 每个** | **0.86 ms** |

**⇒ 两种切法都落在 ~0.85 ms/行。确认：空隙是「按 prompt 行」的线性工作，
与分块方式无关。**（切 64 行时 6 个 pass × 55 ms = 330 ms，与 349 行 × 0.87 = 302 ms 一致。）

**顺带得到一个有用的负面结论**：`QW_PREFILL_CHUNK=64` 让 TTFT 从 1084 ms
**恶化到 2687 ms（2.5 倍）**——64 行一个 pass 要 383 ms，
而 349 行一个 pass 只要 738 ms。**默认的大 chunk 是对的，不要调小。**

**累计已排除**：`logits_row`+argmax（0.5 ms）、逐行采样（已修）、
prefix 快照（28.6 ms 且已禁用）、tail pipeline 编译（A/B 证伪）、
「按 pass 的固定开销」（本轮证伪）。

**剩余的解释**：post-pass 区块里存在一段**每个 prompt 行**都执行的工作。
下一步应在该区块内做二分打点（在 emit 循环前后、snapshot 块前后各加时间戳）。

### 72dt. 空隙定位：一个关键的未决问题（第 186 轮）

在继续二分之前，先确认一件会决定整条路线的事：
**空隙到底是「CPU 侧工作」，还是「GPU 还在跑、计时器量的是入队时间」？**

* 计时点：`step time` 的 `took` 是在 `forward_rows` 返回后立刻量的。
* 如果 `forward_rows` **不等待 GPU**（异步入队），那么 `took` 是入队时间，
  **空隙就是 GPU 排空时间**——那不是「浪费」，而是被错误归属的 prefill 本体。

**支持「排空」的间接证据**：若 439 行 pass 是异步的，下一个 pass 的
`set_tokens`（写 token 缓冲）必须等 GPU 读完它，**排空就正好落在空隙里**，
且**随行数线性增长**——与实测的 0.85 ms/行完全吻合。

**反对「排空」的间接证据**：tail pass 的 post-pass 块里有一次
`logits_row`（文档写明是**阻塞式 GPU 读**），实测只要 **0.5 ms**。
如果前面有 357 ms 的排空，这一次阻塞读会把它吸收掉。
**但 `logits_row` 是在 4 行的 tail pass 之后调用的，不是 439 行的那个 pass**，
所以**这条证据对 439 行 pass 不成立**。

**⇒ 未决。这个问题的答案决定后续方向：**

| 若空隙是… | 含义 | 下一步 |
|---|---|---|
| **GPU 排空** | 不是缺陷，冷 TTFT 的 prefill 本就那么贵 | 应改计时口径，转去优化 prefill 本体 |
| **CPU 侧工作** | 是真的浪费，0.85 ms/行 | 在 post-pass 块内二分定位 |

**判定方法（下一步）**：在 `forward_rows` 内部、提交批次之后立刻打一个时间戳，
与 `forward_rows` 返回时刻比较。若两者相差很小而下一个 pass 的开始明显滞后，
...
### 72dy. 最后一个判别事实：空隙**只出现在大 prefill pass 之后**（第 191 轮）

回看上一轮的日志，有一个之前没注意到的判别事实：

```
step time: 349 rows, ended at  898 ms
   ... 空隙 262 ms ...
step time:   4 rows, started at 1160 ms, ended at 1199 ms
TTFT 1209 ms        <- 4 行 pass 之后只用了 10 ms
```

**⇒ 349 行的 pass 之后有 262 ms 空隙，而 4 行的 tail pass 之后只有 10 ms。**

**⇒ 空隙与该 pass 的「行数」成正比（约 0.75-0.85 ms/行），
而不是与「pass 数」或「prompt 总行数」成正比。**

**这与前面 8 项排除合起来构成一个矛盾：**

* 349 行 pass 之后，**可见的按行代码只有 emit 循环，实测 0.03 ms**；
* 但空隙是 262 ms，即 **0.75 ms/行**——**比可见代码慢 25 倍**。

**⇒ 结论：那段按行的工作要么不在我读过的代码路径里
（例如在 `emit` 之外的某个 `slots` 遍历分支、或 `Active` 的某个 drop/分配），
要么其单次成本远高于表面（例如每次迭代触发了分配或系统调用）。**

**这已经超出「读代码猜」能解决的范围，需要逐段插桩，
而每轮插桩需要重建 + 启服务 2-3 分钟。按剩余轮次预算，收益低于成本。**
...
### 72ea. **`rows>4` 路径有一个约 300 ms 的固定 GPU 成本**（第 193 轮）

上轮关闭了冷 TTFT 那条线，本轮转向**最大的缺口——聚合 otps（Splash 的 46%）**。
之前在 `rows > TILE(4)` 处发现的断崖（4 行 50.8 ms → 8 行 361 ms）一直没查成因。
用 `QW_ENCODE_TIME` 拆开 8 行 pass：

```
batch: CPU encode 66.20 ms | commit+wait 284.57 ms | 1186 dispatches in 898 encoders
batch: CPU encode 57.11 ms | commit+wait 348.95 ms | 1186 dispatches in 898 encoders
```

**⇒ 8 行 pass ≈ 62 ms CPU + ~300 ms GPU ≈ 362 ms，与实测 361 ms 吻合。**
...
| **8（`rows>4` 路径）** | **~300 ms** |
| 16（同上） | ~325 ms |
| 349（prefill，同一路径？） | 810 ms |

**⇒ 349 行才 810 ms，而 8 行就要 300 ms —— 严重不成比例。**

**⇒ `rows>4` 路径有一个约 290 ms 的固定 GPU 成本**（拟合：`290 + 2.2 × rows` ms，
8 行 → 308 ms，16 行 → 325 ms，与实测 300/325 吻合）。

**⇒ 这重新定义了聚合缺口：不是「宽 pass 每行效率低」，而是
「`rows>4` 路径有一次约 290 ms 的固定开销」。**

**潜在收益**：若这 290 ms 能消掉，8 行 pass 约 60 ms
→ 聚合 **8/0.060 = 133 tok/s**（今天 78.8）；16 行约 85 ms → **188 tok/s**。
**这比之前设想的「重写整条路径」要窄得多——它是一个固定的 290 ms，不是每行成本。**
...
### 72ec. **找到并修复了 `rows>4` 的固定开销：`QW_GEMM_MIN_ROWS` 默认从 8 改为 32**（第 195 轮）

上轮把 290 ms 里的 54 ms 归因到 `mpp_src()`，剩余 236 ms 未知。本轮找到了。

**关键推理**：MPP 张量路径的 tile 是 **M(tokens)=128**（见 linear.rs 的注释：
「Tile M(tokens)=128, N(rows)=64, K=32」）。
**而 `QW_GEMM_MIN_ROWS` 的默认值是 8——意味着一个 8 行的 pass 会进入 MPP 路径，
用 M=128 的 tile 去算 8 行，16 倍超配，几乎全部张量算力被浪费。**

**这正是「与行数无关的固定开销」的来源。**

**用已有的开关直接验证（`QW_GEMM_MIN_ROWS`），n=8：**

| `QW_GEMM_MIN_ROWS` | 8 行 pass | 聚合 otps |
|---|---|---|
| 8（旧默认） | **385.4 ms** | 19.40 |
| **32** | **99.5 ms** | **59.80** |
| 1020（永不用 GEMM） | 100.3 ms | 47.16 |

**⇒ 阈值 32 让 8 行 pass 从 385.4 → 99.5 ms（3.9 倍），聚合从 19.40 → 59.80（3.1 倍）。
而 1020 只有 47.16，说明 GEMM 对 32 行以上仍然是对的——32 是合适的阈值。**

**改动**：`gemm_min_rows()` 的默认值 `8` → `32`（一行，含注释说明）。

**正确性验证（三项全绿）：**

* `gemm-check`：worst relative error 3.347e-4 → **PASSED**
* `fastchk`：md5 `5b5f6e931dddd4cc943589f4380e2325` → **与基准逐字节一致**
* `accept.sh`：**19 passed, 0 failed, ACCEPTED**
...
| n | 旧 | 新 | 增益 |
|---|---|---|---|
| 4 | 58.25 / 55.52 | 57.54 / 53.18 | ~1.0×（如预期，不受影响） |
| **8** | **15.67 / 16.01** | **45.41 / 37.70** | **~2.6×** |
| **16** | **23.14** | **32.54** | **1.41×** |

**⇒ 8 路并发快 2.6 倍，16 路快 1.41 倍，全部通过正确性门禁。**

**诚实的边界**：**n=4 不受影响**（4 行本来就低于两个阈值），
所以**目标里对比 Splash 的那个 n=4 聚合指标没有改善**。
这是一个对 8/16 路并发的真实、已验证的改进，但不是目标指标本身的改进。

### 72ed. 阈值修复后重估「多 slot 共享 verify」：**仍然不可行**（第 196 轮）

上一轮的修复（`QW_GEMM_MIN_ROWS` 8→32）让 8 行 pass 快了 3.9 倍，
于是我**重新检查了第 168 轮否掉的「多 slot 共享一次 verify」方案**——
当时否掉的理由正是「16 行 pass 要 385 ms」。
**如果 16 行能降到 ~100 ms，那么「4 个 slot 各 draft 4 个 token、一次 16 行 verify
结算 16 个 token」就是 160 tok/s，正好是目标。**

**实测（修复后的默认值，MAXTOK=64）：**

| rows | pass | ms/行 | 若 1 token/行 |
|---|---|---|---|
| 4 | 50.8 ms | 12.7 | 78.8 tok/s |
| 8 | 126.3 ms | 15.8 | 63.3 tok/s |
| **16** | **277.7 ms** | **17.4** | **57.6 tok/s** |

**⇒ 16 行 pass 不是 ~100 ms，而是 277.7 ms。**

**⇒ 修复后每行成本其实相当平坦（12.7-17.4 ms/行），
所以「16 行 ≈ 8 行的成本」这个外推是错的。**

**⇒ 16 行 verify 结算 16 个 token = 16/0.2777 = 57.6 tok/s，
仍然低于 n=4 的 78.8。**

**⇒ 「多 slot 共享 verify」方案**依然不可行**，第 168 轮的结论成立。**

**更一般的结论**：这套架构下**每个 pass 每个 slot 只产出 1 个 token**，
而 pass 成本大致与行数成正比（12.7-17.4 ms/行），
**所以聚合吞吐在 60-80 tok/s 附近饱和，与并发数基本无关**。
要突破，必须让**每行产出多于 1 个 token**（即在 n>1 时也做投机），
而上面的算术说明**即使做了，16 行的成本也会把收益吃回去**。

**⇒ 聚合 79.4 tok/s（Splash 的 46%）在这套架构下依然没有可行杠杆。**
**上一轮的修复是真实的（n=8/16 快 2.6×/1.41×），但它不改变这个结论。**

### 72ee. 多行 GEMV 的带宽损失：机制已定位，但「4 token」已是寄存器甜点（第 197 轮）

**起因**：注意到一个之前没算过的账。GEMV 是**访存受限**的——
4 行读的是**同一份 14.4 GB 权重**，激活值只多 4 行、可忽略。
**所以 4 行本应耗时 ≈ 1 行（~27-32 ms），而不是实测的 50.8-75.5 ms。**
...
**⇒ 多行核丢了 2.8× 带宽（534 → 191 GB/s）。**
**若修好，4 行 ~30 ms → 4/0.030 = 133 tok/s（今天 79.4）。**

**但读内核宏后，机制比预想的更受约束**（msl.rs:602 附近的注释原文）：

> "The GEMV family above is weight-stationary over at most four tokens: each thread
> keeps one accumulator per token, and **registers run out at k=8**."

**⇒ 关键更正：权重确实是「一次读、四 token 共用」（weight-stationary），
所以 534 → 191 GB/s 的损失**不是重复流量**，而是**占用率（occupancy）**——
每个线程持 4 个累加器会挤掉常驻 threadgroup 数量，从而掩盖不了访存延迟。**

**⇒ 而「4 token」已经是有文档记录的寄存器甜点**（k=8 就溢出了）。

**⇒ 所以「4 行回到 30 ms」并不像上一条推测的那么容易：
要同时做到「权重读一次」和「高占用率」，需要换一种组织方式
（例如权重经 shared memory 在更多线程间共享、或每线程减少累加器但增加线程数），
而这正是我此前反复证伪过的内核级调优领域。**

**⇒ 未做改动，未验证。这是一个有明确机制的目标，但不是「改一行」的目标。**
**下一步候选**：`q4_gemv_k8_u4hx`（NK=8，已实例化，注释说它「让 k=8 这个问题可回答」）
——测它的**每 token** 成本是否优于 k4。若优于，则是一个真杠杆；若相同或更差，
则确认 4 token 就是上限，聚合缺口在这套内核组织下无解。

### 72ef. **重要更正：`K4_U4HX` 在 k=4 时已达 418 GB/s，多行 GEMV 不是瓶颈**（第 198 轮）

上一条（§72ee）我基于「4 token = 75.5 ms = 191 GB/s」推出「多行核丢 2.8× 带宽」。
**本轮用 bench 直接测模型实际使用的那个内核，结果推翻了这个前提：**

```
$ qwen38 bench --tokens 4 --rows 5      # --rows 5 = K_Q4_GEMV_K4_U4HX
one full weight sweep over k=4 tokens, rows=5: 34.47 ms -> 116.0 tok/s linear path, 418 GB/s effective
{"ms_per_token": 8.619, "effective_gbps": 418.1, "weight_bytes": 14412349440, ...}
```

**⇒ `K4_U4HX` 在 k=4 时是 34.47 ms / 4 token = 8.619 ms/token、418 GB/s。**

**⇒ 不是 75.5 ms / 191 GB/s。那个数字来自默认路径（`--rows 0` 那类），不是模型用的内核。**

**⇒ 结论更正：多行 GEMV 已经接近最优**——
418 GB/s 对 1 token 的 534 GB/s 是 78% 效率，
**而每 token 成本从 27.0 ms（1 token）降到 8.619 ms（4 token），是 3.1 倍改善。**

**⇒ 所以 §72ee 的「4 行应该 30 ms」算术要重写：内核本身 4 行就是 34.47 ms，
而模型里 4 行 pass 是 50.8 ms。**

**⇒ 差值 50.8 - 34.5 = 16.3 ms（32%）不在 GEMV 里，
而在非 GEMV 部分**（48 层 GDN + 16 层 attention + norm + conv + 状态读写）。
GDN 状态 1.16 GB，按 481 GB/s 读一次约 2.4 ms——**所以 16.3 ms 偏高，值得查。**

**⇒ 聚合方向的新目标因此更清楚了：不是「修多行核」（它已近最优），
而是「模型里那 16.3 ms 的非 GEMV 开销」。**
...
### 72eh. 16.3 ms 的构成已量化：**1409 个非 GEMV 小内核，每个约 11.6 µs**（第 200 轮）

用 `QW_DISPATCH_HIST` 取一次 4 行 pass 的派发直方图：

```
dispatch histogram, 1906 total:
```

**⇒ 一个 4 行解码 pass 是 1906 次派发**（之前记的 1186 是 prefill 的数字，不是解码）。

**⇒ 其中 497 是 GEMV（bench 的 sweep 覆盖的那部分，34.47 ms），
剩下约 1409 个是非 GEMV 内核。**

**⇒ 16.3 ms / 1409 = 每个非 GEMV 内核约 11.6 µs。**

**⇒ 这 16.3 ms 的构成是「大量极小的内核」**，
分布在 64 层里就是每层约 22 个非 GEMV 派发
（norm、gate_mul、rope、silu、conv、attention 打分/softmax/context 等）。

**⇒ 所以聚合方向的杠杆是**减少内核数量（融合）**，而不是让某个内核更快。**

**这与 §72di 记下的「融合 689 个小内核」是同一个方向，
但现在的数字是精确的：1906 次派发、1409 个非 GEMV、每个 11.6 µs。**

**⇒ 注意这 11.6 µs 已经包含派发开销与 GPU 执行，
而 CPU 侧编码一次 4 行 pass 只要 0.85 ms（§72dv 实测），
所以这 16.3 ms 主要是 GPU 侧的串行执行，不是 CPU 提交。**
...
### 72el. **直方图之谜解开：1906 是 8 行 pass，不是 4 行；`copy_off` 96 = 48 层 x 2**（第 204 轮）

上一轮我说「找不到产生 192 次 `copy_off` 的路径」。**本轮把两个直方图对拆，谜底是：
我读错了 pass 的行数。**

**两个直方图逐内核对比（1906 与 1458）：**

| kernel | 1906 | 1458 | 比值 |
|---|---|---|---|
| `copy_off` | 192 | **96** | **2x** |
| `conv1d_silu_ring` | 192 | **96** | **2x** |
| `gdn_step` | 192 | **96** | **2x** |
| `rope_partial` | 128 | **64** | **2x** |
| `kv_append` | 64 | **32** | **2x** |
| `attn_scores_softmax` | 64 | **32** | **2x** |
| `attn_out` | 64 | **32** | **2x** |

**⇒ 所有随行数变化的内核，1906 那个恰好是 1458 那个的两倍。**

**⇒ 所以 1906 是 8 行 pass，1458 是 4 行 pass。**
**⇒ 我之前用「日志里最近的一条 step time 是 4 rows」去关联直方图是错的
——直方图是在 pass **之后**打印的，我匹配到的是**上一个** pass 的行数。**

**⇒ 于是 `copy_off` 在 4 行 pass 里是 96 次 = 48 个 GDN 层 x 2。**

**⇒ 这**证实**了 `copy_seq` 确实每 pass 在跑**（96 = 48 层 x 2 个缓冲，
正是 `copy_seq` 里那个 `for layer ... for (live, saved, stride) in [state, window]`），
**§72ek 的「没有调用者」结论因此也要修正**——
`save_prefix`/`load_prefix` 虽是 `pub` 且没有 Rust 调用点，
**但 `copy_seq` 显然被某处调用了**（否则不会有这 96 次派发）。**

**⇒ 同时 §72ei 的「11.6 µs/内核」是**基于 8 行 pass**算的（16.3 ms 是 4 行 pass 的），
所以那个数字也需要重算。**
...
### 72eq. **融合核实现后失败并回退——同时暴露出 `accept.sh` 的一个覆盖盲区**（第 209 轮）

按 §72ep 的设计实现了多序列融合核（`conv1d_silu_ring_tile_multi`，
按行以标量传入 pos 与 window 偏移），替换 `else` 分支里的「逐行拷贝 + 逐行卷积」。
...
| | 生成 token 数 | 聚合 otps |
|---|---|---|
| 旧（`/tmp/q_u4hx`） | **512** | 66.93 / 61.92 |
| 新（融合核） | **129 / 131** | 25.71 / 19.73 |

**⇒ 新构建在 n=4 下只生成 129 个 token 就停了（应为 512），输出是错的。**

**⇒ 已立即 `git checkout crates/` 回退，工作树干净，已重建回 HEAD。**

**⇒ 关键发现：`accept.sh` 的 19 项门禁**全部通过了一个在 n=4 下产生错误输出的构建**。**
**这是测试套件的一个真实盲区——它没有覆盖「批解码的 conv ring 更新」这条路径。**
**`fastchk` 是 n=1，`gemm-check` 只看单层权重，`accept.sh` 的并发用例显然也没有
捕捉到 ring 状态被写坏。**

**⇒ 这个盲区本身比这次失败的改动更值得记录**：
在此之前，我一直把 `accept.sh` 19/0 当作「改动安全」的充分证据。
**这一轮证明它不是**——至少对批解码路径不是。

**⇒ 失败的可能原因（供下次参考）**：
（1）`conv_out` 的写入布局——旧代码用 `buf_offset(2, &scratch.conv_out, row*(conv_dim*2))`
    逐行偏移，新代码用 `buf(2, &scratch.conv_out)` 让核按 `row*conv_dim+c` 自行索引，
    **若 `conv_out` 的实际布局不是紧密的 row-major half，这里就错了**；
（2）`qkv_cur` 的行布局是否与 `row` 索引一致；
（3）`pos & (ring-1)` 与旧代码 `slot = pos % conv_ring` 在 pos 为负数时的差异
    （旧代码用 `%`，新代码用 `&`，而 `conv_ring` 是 2 的幂所以通常等价，
     但**负 pos 下 C 的 `%` 与 `&` 行为不同**）。

**未保留任何未验证的改动。下一步需要先补一个能捕捉这个 bug 的门禁，
再谈实现——否则每次尝试都无法判断对错。**

### 72er. **补上门禁并证明它能抓住那个 bug**（第 210 轮）

§72eq 暴露出 `accept.sh` 会通过一个批解码错误的构建。本轮补上门禁并**证明它有效**。

**门禁设计（`tools/accept.sh`，现为 20 项）：**

> 同一个 prompt 并发跑 4 次，必须与单独跑一次的输出**逐字节一致**。

* 单独那一次**先在独立服务上取**，避免它自己被污染；
* 用 `temperature=0`；
* 之所以用「并发 vs 单独」这个不变量，而不是「`finish_reason` 必须是 length」——
  **后者我第一版就是这么写的，结果在 HEAD（正确代码）上就红了**，
  因为模型可能合法地提前停止。**不变量必须是对正确代码也成立的。**

**证明它能抓住 bug（关键一步）：**

把 §72eq 那个坏掉的核**原样重新应用**，重新构建，跑 `accept.sh`：
...
**⇒ 4 个并发请求全部与单独请求不一致，门禁如实报错。**

**然后 `git checkout crates/` 回退，重建，再跑：**
...
**⇒ 门禁在正确代码上绿、在目标 bug 上红——它被证明能抓住它要抓的东西。**

**⇒ 这一步很重要**：否则我无法区分「门禁通过」与「门禁根本没在测这件事」。
**第一版门禁就属于后者**（它在正确代码上就失败），如果我没验证就提交，
会得到一个永远红着的假门禁。

**交付状态**：`accept.sh` 20/0 在 HEAD 上通过；工作树干净；
**新增的门禁已提交（`c8915c8`），坏改动未保留任何痕迹。**

### 72es. **失败根因定位：我把 §72ep 明确要求的 `row_cur` 映射简化掉了**（第 211 轮）

有了能判对错的门禁后，重新审视 §72eq 的失败。**根因是我自己引入的，而且 §72ep 早就写明了。**

**§72ep 的设计里明确要求「三个按行数组」，其中一个是 `row_cur[R]`——
「第 R 行自己在 `cur` 中的行号」，并特别指出
「多序列版本必须把 `cur` 的下标也改成按行查表，
而不能沿用 `pos - pos0` 这个『位置偏移即行号』的巧合」。**

**而我在实现时把它简化掉了**，核里只对 `pos == p0` 这一个 tap 读 `cur`：

```c
const int pos = p0 - 3 + j;
device const half* src = (pos >= p0)
    ? cur + (size_t)row * conv_dim + c        // 只有 j==3 才走这里
    : window + (size_t)(wo + (pos & (ring-1)) * conv_dim) + c;
```

**⇒ 这假设了「每行所属序列在本 pass 内只有这 1 行」。**

**但 `else` 分支的条件是 `!rows.iter().all(|r| r.0 == rows[0].0)`——
即「**不是所有行都同序列**」，**并不等于「所有行都不同序列」**。**

**⇒ 反例：一个 pass 有 4 行 = 序列 A 的 2 行 + 序列 B 的 2 行。**
**此时 `all same seq` 为假 ⇒ 走 `else` ⇒ 我的核。
而序列 A 的第 2 行，它的 tap 需要序列 A 第 1 行的数据——
那份数据在 `cur` 里，**还没写进 window**，而我的核会去读 window ⇒ **读到旧值**。**

**⇒ 这就是 n=4 下生成 129 而非 512 个 token 的原因：ring 被写坏/读错。**

**⇒ 正确修法（回到 §72ep 的设计）：每行传 `pos0[R]`（该序列在本 pass 的起始位置）
与 `cur_base[R]`（该起始位置对应的 `cur` 行号），核内：**

```c
const int pos = p0 - 3 + j;
const int q   = pos - pos0_row;              // 该序列内的行偏移
device const half* src = (q >= 0)
    ? cur + (size_t)(cur_base_row + q) * conv_dim + c
    : window + (size_t)(wo + (pos & (ring - 1)) * conv_dim) + c;
```

**⇒ 对「每序列 1 行」的情形，`pos0_row = p0`、`cur_base_row = row`、`q = 0`，
退化为我原来写的形式——所以原来的写法在纯解码下是对的，
**只有「同序列多行」时才错**。这与「n=4 纯解码时 4 个 slot 各 1 行」
看似矛盾，说明实际触发的场景是**并发预填**（多个请求同时预填，
...
### 72ev. 融合批量卷积：做出来了、证明正确了，但收益只有 1.5%（第 180 轮）

#### 先关掉三条路

**(1) 把权重驻留内核加宽到 8/16 token —— 关闭。**
`qwen38 bench` 交错三次，每次 `--iters 3`：

| 臂 | 时间 | ms/token | 有效带宽 |
|---|---|---|---|
| `--rows 5 --tokens 4`（`K4_U4HX`） | 39.7 ms / 4 token | **9.93** | **362 GB/s** |
| `--rows 6 --tokens 8`（`K8_U4HX`） | 94.7 ms / 8 token | 11.83 | 152 GB/s |

**⇒ k8 每 token 更慢，带宽掉到 42%。** `msl.rs:602` 的注释「registers run out at k=8」
在实测中成立，而且 k=8 不只是到顶，是**已经变差**。

**(2) 行摊销 GEMM 在小行数上不摊销 —— 关闭。**
`--rows 8`（`K_Q4_GEMM_TILE`）：16 token → 17.70 ms/token；32 → 11.73；64 → 7.73。
**16 行时 GEMM 比 tile 循环（13.31 ms/token）更差**：M=128 的 tile 在 16 行时 8× 空转。

**(3) pass 成本严格随行数线性，4 行以上零摊销。**
`pos-bench --slots 16`（每序列 1 行，即批量解码形状）：

| rows/pass | ms/pass | ms/token | tok/s |
|---|---|---|---|
| 4 | 54.42 | 13.61 | 73.5 |
| 8 | 106.51 | 13.31 | 75.1 |
| 16 | 204.07 | 12.75 | 78.4 |

**⇒ 4 : 8 : 16 = 1 : 1.96 : 3.75，聚合 otps 被钉在 73–78，与并发度无关。**

#### 天花板算术（这是本轮最重要的结论）

一次权重扫描 = 14.412 GB。k=1 内核实测 464 GB/s ⇒ 31.05 ms/扫描；
理论上限 481 GB/s ⇒ 29.96 ms/扫描。`TILE=4` ⇒ **每次扫描最多 4 个 token**。

| 情形 | 4 行 pass | 聚合 otps |
|---|---|---|
| 现状（实测，`slots=4 rows=4`） | 54.4 ms | **73.5** |
| k4 内核达到 k1 的带宽（362 → 464 GB/s） | 45.8 ms | 87.4 |
| 再加上非 GEMV 开销全部消失 | 31.1 ms | **128.8** |
| 481 GB/s 理论极限 | 30.0 ms | **133.5** |
| **Splash 声称（4 并发 SHORT）** | — | **170** |

**⇒ 170 tok/s 需要每次扫描 5.3 个 token，或 >800 GB/s 的有效带宽。
在 TILE=4 + 481 GB/s 下两者都不成立。** 这条差距不是调优能补的。

#### 方法论：`batch-check` 的 `worst |logit difference|` 是不确定的

**同一份未改动的基线二进制、同一个 217-token prompt、同一个 `--slots 4`，连跑 4 次：**

```
worst |logit difference| 0.0000
worst |logit difference| 0.0156     <-- 基线自己
worst |logit difference| 0.0000
worst |logit difference| 0.0000
```

**⇒ 这个数字不能当回归信号。** 能当门槛的只有 greedy token 相等（4 次全过）。
本轮一开始就是被这个数字误导的：融合版在 1081-token prompt 上给出 0.0156，
而基线在另一个长度上是 0.0000 —— 看起来像回归，实际是门槛在抖。
**先跑基线对照，才没有误判。**

#### 融合卷积：实现

按 §72eu 的推断做成一般版本 `conv1d_silu_ring_multi`（`msl_ops.rs`）。
每行从 `conv_meta`（i32，4 个/行）读 `(p, gp0, gc0, wo)`：

```
meta[row*4+0] = p     本行位置
meta[row*4+1] = gp0   本行所属序列在本次 pass 的第一个位置
meta[row*4+2] = gc0   gp0 对应的 staging 行号
meta[row*4+3] = wo    该序列的 window 偏移（以 ELEMENT 计，win_stride 是字节）
```

tap 在 `pos >= gp0` 时从 staging buffer 读（是本 pass 的另一行），否则从 ring 读 ——
和 `conv1d_silu_ring_tile` 同一条规则，只是每序列各有一份 `pos0` 和 window。
**关键：`gc0` 是传进去的，不是用位置算出来的。**
`cur` 是 pass 顺序而位置顺序不同，`base + offset` 会读到别的请求的行 ——
这正是前两次失败的根因（§72es、§72eu），从根上消失。
环更新照旧折在里面，所以调用方**不能**先拷贝：每层 8 次派发（4 copy + 4 conv）变成 1 次。

调用方（`runner.rs` 的 `else` 分支）按序列分组构造 `conv_meta`，
并**校验**每组位置连续、每序列只出现一组；不满足就回落到原来的逐行 pair，
这样一个异常调用方不会静默拿到错答案。
...
| 门槛 | 结果 |
|---|---|
| `gemm-check` | **PASSED**，最差 rel 3.347e-4 |
| `batch-check` 2 / 4 / 8 / 16 slots | 全过，greedy token 全等 |
| `batch-check` 1081-token prompt（**超过 1024 的 ring，覆盖回绕**） | 过，4/4 |
| `accept.sh` | **20 passed / 0 failed ACCEPTED**，含第 20 关「四个并发相同 prompt 与单请求逐字节相同」 |
| 派发直方图（确定性） | **1906 → 1570**；`copy_off` 192 + `conv1d_silu_ring` 192 → `conv1d_silu_ring_multi` **48** |

#### 收益：1.5%，不是 18%

交错 A/B（`pos-bench`，机器处于快状态，min/p25/median 同向）：

| 臂 | 基线 | 融合 | 比值 | 配对胜 |
|---|---|---|---|---|
| **处理** `slots=4 rows=4`（混合序列，改动生效） | 14.58 | 14.37 | **1.015×** | **6/7** |
| **对照** `slots=1 rows=4`（同序列，未改动） | 12.54 | 12.55 | 0.999× | 2/7 |

对照臂不动 ⇒ 这 1.5% 是内核的功劳，不是时钟漂移。
（在热降频状态下另跑过 3 次，都是 ~1.003× —— 降频会掩盖小收益，与这里一致。）

#### 为什么只有 1.5%：上一轮的归因是错的

本轮开始时测出 `slots=1 rows=4` = 46.1 ms、`slots=4 rows=4` = 54.9 ms，
把 9 ms 记在「逐行 copy+conv」头上。**直方图显示这个归因是混淆的：**

| 内核 | `slots=1 rows=4` | `slots=4 rows=4` | 差 |
|---|---|---|---|
| 总派发 | 1186 | 1906 | +720 |
| conv 路径 | 48 (`conv1d_silu_ring_tile`) | 384（192 copy + 192 conv） | **+336** |
| `gdn_step` | 48 (`gdn_step_seq4`) | 192 | +144 |
| `rope` | 32 (`rope_partial_rows`) | 128 | +96 |
| `kv_append` | 16 (`kv_append_rows`) | 64 | +48 |
| `attn` | 16+16 (`*_rows`) | 64+64 | +96 |

批量 pass 比同序列 pass 多 **720** 次派发，conv 只占 **336**。
而把 336 次拿掉，时间**没变**（三次交错 A/B 都是 ~1.00×）。

**⇒ 这些派发不在关键路径上。** 4 行 pass 的 39.7 ms 权重扫描期间，
小的逐行派发被内存停顿掩盖了。**「融合卷积值 9 ms」是错的，它值 1.5%（≈0.2 ms）。**

#### 结论

- 融合内核**正确，且是 1.5% 的真实提升**（有对照的交错 A/B + 全部正确性门槛），**保留并提交**。
- 但它**不是**通往 Splash 170 tok/s 的路。那条路被上面的天花板算术封死：
...
## §72ez otps 的墙到底在哪：k4 的 366 GB/s，以及一个「微基准说好、端到端说坏」的坑

冷/热 TTFT 在 §72ey 已经和 llama.cpp 持平，这一轮只剩 otps（我们 ~58，Splash 170）。
先确认几件本来会猜错的事。

### 并发解码其实已经在批处理

4 路并发时引擎的 `step time` 是 **4 rows**，一个 pass 一次权重读。不是「每个请求一个 pass」。
pass 之间的空档：94 个 pass 统计，**均值 6.1 ms、中位数 2.2 ms**（pass 本身 52.5 ms，轮次 58.6 ms）
—— 引擎没有闲置，52.5 ms 就是成本本身。

### spec 在多路时被关掉，而且这是对的

`engine.rs:1017`：`spec_slot` 只在 `decoding == 1` 时才有值，所以 n>1 时全部走普通批处理。
一开始以为这是「Splash 有 spec、我们没有」的差距，算一下发现**关掉是对的**：

spec 的一个 pass 是 TILE=4 行，settle 约 2.7 个 token（58% 接受率），**即每行 0.675 个 token**；
而普通批处理一行就是一个 token。pass 时间 ≈ 55 + 13·rows ms（实测 4 行 52.5、8 行 119.8、16 行 158、
52 行 474），所以：

| | 每 pass token | r=4 | r=16 |
|---|---|---|---|
| 普通批处理 | r | 37 tok/s | 61 |
| 带 spec | 0.675r | **25** | **41** |

**r≥4 时 spec 反而更慢。** 这也解释了为什么「multi-slot shared verify」当初被否掉。

### MPP 在解码行数下更差，任何 tile 都一样

`QW_GEMM_MIN_ROWS=8` 把 8 行批处理改走 MPP（NRB=64/128/256/512 都试了）：
...
MPP 的 tile 会按 NRB 补齐，8 行配 64 的 tile 就是 8 倍空转。**解码就该用 GEMV。**

### 真正的墙：k4 已经接近带宽上限，k8/k16 崩掉

| kernel | token | 时间 | ms/token | 有效带宽 |
|---|---|---|---|---|
| k1 | 1 | 30.16 ms | 30.16 | **477.9 GB/s**（贴着 481 的天花板）|
| k4 | 4 | 39.52 ms | **9.88** | **364.7 GB/s**（天花板的 76%）|
| k8 | 8 | 100.67 ms | 12.58 | 143.2 GB/s |
| k16 | 16 | 432.72 ms | 27.0 | **33 GB/s** |

**k4 是这条路的极限**：366 GB/s 已经接近 478。4 行 pass 52.5 ms → 聚合上限 76 tok/s，我们实测 58。

Splash 的 170 需要 ~5.9 ms/token。4 个 token 要做到 23.6 ms —— 比 478 GB/s 读一遍 14.4 GB（29.96 ms）还快，
**物理上不可能**。唯一的可能是 8 个以上 token 一次权重读且保持 k4 的带宽效率：
8 token × 4.94 ms = 39.5 ms → **202 tok/s**。今天 k8 只有 187 GB/s，是 k4 的 51%。

**所以 otps 的下一步很明确：把 k8 的带宽效率从 187 GB/s 做到 k4 的 366 GB/s。**

### 一个坑：微基准说 +3.1%，端到端说 −20%

顺手试了给权重解包循环去掉完全展开（`_Pragma("unroll 1")`，寄存器压力是 k8 崩掉的公认原因）。
隔离测量非常干净、三次交错全部支持它：

| | k4 ms/token | k8 ms/token |
|---|---|---|
| 改前 | 9.908 / 9.916 / 9.902 | 12.584 |
| 改后 | **9.542 / 9.516 / 9.767** | **9.613** |

k4 提升 3.1%，k8 提升 31%，k1 不变，而且 accept.sh 20/0、gemm-check、batch-check 全绿。

**但真实解码 pass 变慢了**，六次运行、顺序交替（old,new,new,old,old,new）统计 4 行 pass 的最小值：

| | min（三次） |
|---|---|
| 改前 | **51.25 / 67.19 / 65.85** |
| 改后 | 63.31 / 75.47 / 71.65 |

改前的三次**全部**优于改后的三次，连「改后先跑」的那两次也是。**已回滚。**

原因大概是 `bench --rows 5` 把 497 个线性层编进**一个 command buffer、输入是常量**，
是纯流式上界；真实 pass 有依赖链，减少展开带来的循环开销在这里反而更贵。

**教训：`bench --rows` 的线性上界不能替代真实 pass 的判断。**
`end_to_end: false` 就写在它的输出里。

---

## §72fa 解码瓶颈不是权重带宽，是 x 的读取方式：平铺分区 1.075×

### 线索就在代码自己的注释里

`msl.rs` 在 U4HX 宏上方写着：

> The x loads dominate the kernel's memory ops: per (group, word) the row loop issues two
> 8-byte half4 loads per row, while the weights need only one 16-byte load for the whole
> pass - **28.8 G loads against 3.6 G**.

而宏里 x 的取址是 `x + g * GROUP_SIZE + (wi * 4 + c) * 8`，**lane g 取 g×64**：
一个 warp 的 32 条 lane 落在 32 条不同的 128 字节 cache line 上，每条只用 16 字节。
**权重那边是连续的**（lane g 取 `wp + g*2` 个 uint4 = 每 lane 32 字节连续），
所以慢的不是权重，是 x 的**跨步**读取。

### 平铺分区

让 lane L 处理第 `j = iter * 32 + L` 个「8 权重块」，而不是整个 64 权重组：

| | 组分区（旧） | 平铺分区（新） |
|---|---|---|
| x 取址 | `g * 64 + …`，warp 跨步 64 字节 | `j * 8`，**warp 连续 512 字节** |
| 权重取址 | 连续（32 B/lane） | 连续（4 B/lane） |
| scale/bias | 每组 1 次 | 每块 1 次（8×，但每行只有 K/64 个 ushort = 160 B，全在 cache） |

累加顺序变了（这是不同的舍入，不只是不同的调度），所以正确性必须重新证明。

### 隔离测量（各两次，几乎一致）

| kernel | 改前 ms/token | 改后 ms/token | 有效带宽 |
|---|---|---|---|
| k4 | 9.860 / 9.899 | **8.824 / 8.834** | 365 → **408 GB/s** |
| k8 | 11.736 / 11.788 | **7.731 / 7.738** | 153 → **233 GB/s** |

k4 快 1.12×（408 GB/s = 478 天花板的 85%），k8 快 1.52×，
**而且 k8 平铺后（7.73）已经比 k4 平铺（8.83）更省**。

### 真实 pass（这才是判据）

冷却 300 s 后，顺序 old,new,new,old，4 行 pass：

| build | min | p25 | mean |
|---|---|---|---|
| 旧（组分区） | 50.67 | 51.67 | 52.11 |
| **新（平铺）** | **47.19** | **47.92** | **49.97** |
| **新（平铺）** | **47.53** | **47.78** | **48.57** |
| 旧（组分区） | 51.17 | 52.11 | 52.82 |

**1.075×**，两次新都优于两次旧。n=4 聚合 tok/s（同样顺序交替）：
...
正确性：**1390 token prompt 的 64 个 greedy token 与旧 build 完全相同**；
gemm-check PASSED（3.347e-4）；batch-check 4/16 PASSED；accept.sh **20/0 ACCEPTED**。

### 又踩了一次同样的坑

第一次测真实 pass 时机器已经严重降频（pass 均值 148–189 ms，而稳定值是 52.5），
结果「旧」三次 min 51.5 / 80.0 / 75.6 全胜「新」的 93.2 / 93.0 / 86.0，
看起来平铺是退步。**冷却 300 s 后结论完全反过来。**

**即使做了顺序交替，降频状态下的 min 仍然不可信** —— 它是「哪一次恰好没被限频」，
不是 kernel 的性质。**判断 kernel 改动必须先把机器冷却到 pass 时间回到已知的稳定值。**

### 下一步

k8 平铺（7.73 ms/token）已经比 k4 平铺更省：8 行用 k8 是 61.8 ms，
而现在的 2×k4 是 70.6 ms，**约 1.14×**。这只影响 8–31 行（8 路以上并发），
n=4 时行数仍是 4，所以对 Splash 那个「4 并发」的对照没有直接帮助 ——
但它需要真实 pass 的验证，不能只看 `bench --rows 20`。

---

## §72fb 解码 pass 的 1570 次派发，以及 k8 平铺为什么不成立

### 12 ms 不是 CPU，是 GPU

`QW_ENCODE_TIME=1` 在 4 行解码 pass 上给出：

```
batch: CPU encode 0.85 ms | commit+wait 46.94 ms | 1570 dispatches in 1042 encoders
```

**CPU 编码只有 0.7–1.1 ms**，47 ms 全在 `commit+wait`。所以那 12 ms（47.4 减去
纯线性的 4×8.83 = 35.3）是 GPU 真的在执行 1073 个小 kernel，平均每个 11 µs。

`QW_NO_ENC_SPLIT=1`（不安全，只看时间）把 encoder 数从 1042 降到 1：

| | dispatches / encoders | commit+wait |
|---|---|---|
| 正常 | 1570 / 1042 | 48.68 / 47.72 ms |
| 单 encoder | 1570 / 1 | 49.01 / 47.52 ms |

**encoder 数量完全不影响时间**，所以「合并 encoder」不是方向。

### 真正多出来的 384 次派发：解码路径没有按行批处理

602 行 prefill（1186 次）对 4 行解码（1570 次），逐 kernel 对比：

| kernel | prefill | decode | 倍数 |
|---|---|---|---|
| `gdn_step` / `gdn_step_seq4` | 48 | **192** | ×4 |
| `rope_partial` | 32 | **128** | ×4 |
| `attn_out` | 16 | **64** | ×4 |
| `kv_append` | 16 | **64** | ×4 |
| `attn_scores_softmax` | 16 | **64** | ×4 |

prefill 用的是**按行批处理**的 `*_rows` / `gdn_step_seq4`（每个一次，覆盖所有行），
解码用的是**按序列**的版本，4 路并发就是每个序列各派发一次。
1073 − 144 − 96 − 48 − 48 − 48 = **689**，正好等于 prefill 的非线性派发数。

这 384 次多出来的派发就是空间所在。用 `QW_SKIP_KERNEL` 直接量单次小派发的代价
（4 行 pass 的 min）：

| | 跳过的派发数 | 4 行 pass min | 每次 |
|---|---|---|---|
| 基线 | 0 | 49.88 | — |
| 跳过 `rmsnorm` | 129 | 48.03 | **14.3 µs** |
| 跳过 `ewise_add` | 128 | 48.29 | **12.4 µs** |

**每次小派发约 13 µs**，所以 384 次约 **5 ms**，即 1.11× 的空间 —— 但需要把这几个
kernel 改成一次覆盖多个序列（每行各自的 pos / conv state），是较大的改动。

### k8 平铺：隔离更快，真实 pass 更慢，已回退

k8 平铺在隔离扫描里比 k4 平铺更省（7.731/7.738 对 8.824/8.834 ms/token），
于是接进 `encode_rows`：8 行以上改用 8 行一块。

`batch-check --slots 8/16` 都 PASSED，直方图确认 8 行 pass 确实走
`q4_gemv_k8_flat`（497 次）。**但真实 pass 变慢了：**

| 8 行 pass | min | p25 | median | mean |
|---|---|---|---|---|
| 2 × k4 平铺（现状） | **95.24** | 105.47 | 111.15 | 114.38 |
| k8 平铺 | 122.00 | 126.46 | 130.18 | 130.55 |

**1.28× 更慢**，min/p25/median/mean 四个统计量一致。n=8 聚合 tok/s 也没有提升
（旧 59.1/51.2，新 51.8/47.7）。**已回退**，`encode_rows` 保持 4 行一块。

### 第三次：`bench --rows` 预测不了真实 pass

这是第三次隔离扫描和真实 pass 结论相反（第一次是 `unroll 1`，第二次是降频下的
平铺对比，这次是 k8）。规律很清楚：

- 隔离扫描是**同一个 kernel 连续跑 497 次**，GPU 能把它们流水起来，x 还是共享的热 buffer；
- 真实 pass 里每个 kernel 后面跟着别的 kernel，寄存器压力大的 kernel 占用率低，
  换场代价藏不住。
...
## §72fe 天花板到底在哪里：一次 pass 读一次权重，而 spec 只服务单槽

这一节推翻我之前的一个错误结论，并给出真正的杠杆。

### 先修正：spec 解码只对「只有一个槽在解码」时生效
...
**所以 4 路并发时根本没有投机解码**，每个 pass 每个流只出 1 个 token。
这解释了实测的「每流每 pass 约 1 个 token」。

单流（`QW_SPEC_DEBUG=1`，192 token，72 个 pass）：

| 项 | 值 |
|---|---|
| 每 pass 结算 | **2.67 个 token**（1 + 1.667） |
| 接受率 | **55.6%**（0 个 18.1%，1 个 29.2%，2 个 20.8%，3 个 31.9%） |
| draft | 8.25 ms |
| verify | 42.90 ms |
| 单流吞吐 | 47.5 tok/s |

4 路并发纯解码：4 token / pass，63.2 tok/s 聚合。**所以并发时「不用 spec」反而是对的** ——
spec 单流每 pass 2.67 个 token，比 4 个流各出 1 个还少。

### 真正的硬约束：权重每次 pass 只读一次，但每 4 行就要重读一次

权重 14.412 GB，实测带宽上限 478 GB/s：

**30.16 ms —— 这是任何 pass 的下限，与 token 数无关。**
k1 kernel 实测 30.16 ms/token，完全吻合。

但 `encode_rows` 对 `rows > TILE(4)` 是**按 4 行一组循环**，每组都是一次完整权重扫描
（MPP 要 `rows >= QW_GEMM_MIN_ROWS`，默认 32 才走）。于是：
...
实测 16 行 pass：**min 291 ms，而把 `q4_gemv_k4_flat` 跳过之后只有 9.06 ms** ——
**非线性部分在 16 行时也才 9 ms，291 ms 里几乎全是线性**。

强行把 16 行路由到 MPP（`QW_GEMM_MIN_ROWS=16`）反而更慢：**276.53 → 568.06 ms**。
（再次确认「小行数走 GEMM」是死路。）
...
（注：这三次运行时长差很多，有热节流混淆；但 `ms/row` 从 4 行的 11.15 涨到 16 行的 19.29 是 min 值，不受节流影响。）

### ALU 侧的天花板

4-bit 权重 = 0.5 byte/权重，全模型 28.8 G 权重。每个权重对每个 token 做一次 MAC：

- ALU 时间 = 28.8 G × tokens / 5 TMAC/s = **5.76 ms × tokens**
- 显存时间 = **30.2 ms**（与 tokens 无关）

| 每 pass token 数 | max(显存, ALU) | 上限吞吐 |
|---|---|---|
| 1 | 30.2 | 33 |
| 4 | 30.2 | **132** |
| 8 | 46.1 | **174** |
...
**结论：这台机器的 otps 上限约 174 tok/s，出现在「每 pass 8–16 个 token」处，而且是 ALU 受限的。**
Splash 的 170 tok/s 几乎正好压在这个上限上 —— 它是 ALU 最优的，我们是显存/分块受限的。

### 于是杠杆只有一个

**让一次权重扫描服务 8–16 行。** 现在做不到，原因是两件事同时成立：

1. `encode_rows` 按 4 行分块，每块重扫权重。
2. `q4_gemv_k8_flat`（8 行一次扫描）实测在真实 8 行 pass 里**更慢**（122.00 vs 95.24 ms）。
   原因在 `msl.rs:594` 那段注释里已经写过了：**x 的 load 会主导 kernel 的访存** ——
   NK 越大，每读一个权重词就要读 NK 个 x 值。NK=4 时是 1:4，NK=8 时是 1:8。

**所以下一步不是「加宽 NK」，而是做一个 `(features × tokens)` 二维 tile 的 kernel：**
每个线程负责 2 个输出特征 × 4 个 token（8 个累加器），
每次迭代读 2 个权重词 + 4 个 x 值 = 6 次 load 换 8 次 MAC，
而不是现在的 1 个权重词 + 4 个 x 值换 4 次 MAC。

这条路要重写 k4-flat 的分块方式，是这一轮没能完成的工程。
**在它完成之前，4 路并发的上限就是 4 / 30.2 ms = 132 tok/s，而我们已经到 63.2。**

### 本轮没有交付提速

本轮交付的是**测量和定位**：修掉了一个错误结论（「170 不可能」是错的 —— 它只是
在「每 pass 4 个 token」的前提下不可能），并指出唯一可行的方向。
`accept.sh` 21/21 未受影响。

## §72ff 二维分块（features × tokens）交付 1.30×，并修掉一个把 grid 和 kernel 拆开的 bug

§72fe 结尾指出的唯一杠杆是「每个线程负责 2 个输出特征 × 4 个 token」的二维分块。
这一轮把它做出来了，并且在做它的过程中挖出一个**已经出货三轮的正确性 bug**。

### 一、原理：平铺 kernel 的 x 读取是完全冗余的

`Q4_GEMV_KS_FLAT` 一个 threadgroup 负责**一个**输出特征、NK 个 token。
权重一次 sweep 就够，但**激活是每个输出特征都重读一遍**：
每个 lane 每个 j 迭代要读 NK 个 16 字节的 uint4 x，而权重只有 4 字节。

按字节算每个 lane 迭代（K=5120）：

| kernel | 权重 | x | 合计 | 覆盖的 (token × 输出行) | 每单元字节 |
|---|---|---|---|---|---|
| k4-flat (NR=1, NK=4) | 4 B | 64 B | 68 B | 4 | 17.0 |
| k8-flat (NR=1, NK=8) | 4 B | 128 B | 132 B | 8 | 16.5 |
| t28 (NR=2, NK=8) | 8 B | 128 B | 136 B | 16 | **8.5** |
| t44 (NR=4, NK=4) | 16 B | 64 B | 80 B | 16 | **5.0** |

**k8-flat 为什么输**：它每个单元的字节数（16.5）和 k4-flat（17.0）几乎一样，
所以它没有变快，只是把 x 流量翻了一倍去换两倍的 MAC。
实测两条 kernel 都撞在同一面墙上：

| kernel | 一次 sweep 覆盖 | 时间 | 搬运字节 | 有效带宽 |
|---|---|---|---|---|
| k4-flat | 4 token | 35.3 ms | 245 GB | ~7 TB/s |
| k8-flat | 8 token | 61.8 ms | 475 GB | ~7.7 TB/s |

也就是说 14.412 GB 的权重只占搬运量的 6%，瓶颈是 L1/L2 的 load 带宽。
**NR 才是那个把冗余除掉的维度**，NK 不是。

### 二、隔离测量：全 497 个 linear 的一次完整 sweep

在同一个进程里轮转各 arm（`bench --rows 21`），第一个 arm 在最后重复一次，
用来标定仪器自身的偏差。

4 token：

| arm | ms | tok/s | vs base | 校准后 |
|---|---|---|---|---|
| k4-flat ×1（今天的引擎） | 36.24 | 110.4 | 1.0000 | 1.0173 |
| t24 (NR=2) ×1 | 35.30 | 113.3 | 0.9741 | 0.9910 |
| t44 (NR=4) ×1 | 38.30 | 104.4 | 1.0567 | 1.0750 |
| k4-flat ×1（重复） | 35.62 | | 0.9830 | 1.0000 |

8 token：

| arm | ms | tok/s | vs base | 校准后 |
|---|---|---|---|---|
| k4-flat ×2（今天的引擎） | 71.02 | 112.6 | 1.0000 | 1.0123 |
| k8-flat ×1 | 63.10 | 126.8 | 0.8885 | 0.8995 |
| **t28 (NR=2) ×1** | **55.34** | **144.6** | **0.7792** | **0.7888** |
| t48 (NR=4) ×1 | 92.31 | 86.7 | 1.2998 | 1.3158 |
| k4-flat ×2（重复） | 70.16 | | 0.9878 | 1.0000 |

16 token：

| arm | ms | tok/s | vs base | 校准后 |
|---|---|---|---|---|
| k4-flat ×4（今天的引擎） | 140.27 | 114.1 | 1.0000 | 1.0014 |
| t44 (NR=4) ×4 | 145.23 | 110.2 | 1.0354 | 1.0368 |
| t48 (NR=4) ×2 | 183.57 | 87.2 | 1.3087 | 1.3104 |
| **t28 (NR=2) ×2** | **107.80** | **148.4** | **0.7685** | **0.7696** |
| k4-flat ×4（重复） | 140.08 | | 0.9987 | 1.0000 |

结论：**NR=2 / NK=8 在 8 和 16 token 上都是 1.28–1.30×；NR=4 会溢出**
（t48 有 32 个累加器，实测比 2×t28 还慢），**4 token 上 t24 是中性**。

注意 t48 的 92.31 ms **比 2×t28 的 110.68 ms 快** —— 它输给的是 8-token 的
k4 基线（71.02），不是输给 t28。8 token 用 t28，16 token 用 2×t28。

### 三、真实 16 行解码 pass：1.30×

16 路并发时，prefill 之后每个 pass 都是 16 行 —— 这是仍然走 tile 循环的最宽形状
（32 行以上走 MPP GEMM）。冷却 + 交替 + 双向，每 arm 284 个 pass：

| arm | min | p25 | median | mean |
|---|---|---|---|---|
| narrow (4 × k4-flat) | 139.35 | 190.36 | 215.69 | 206.70 |
| wide (2 × t28) | 117.78 | 138.47 | 159.48 | 154.06 |
| **wide/narrow** | **0.8452** | **0.7274** | **0.7394** | **0.7453** |

**墙钟聚合吞吐 69.1 → 89.5 tok/s。**

### 四、顺带修正一个错误结论：并发越高吞吐越高

§72fe 之前记录的「聚合吞吐随并发下降（4→63.2，8→49.7，16→35.9）」是错的。
冷却后重测（wide kernel 已在）：
...
之前那组数字是被连续测量之间的热降频污染的。**4 路并发根本用不到宽 kernel**
（4 行 → k4-flat），所以 wide tile 只对 8 路以上有意义。

### 五、接线策略：用实测常数逐 dispatch 决定

一次 8 行的 sweep 不是免费的（55.34 ms vs 4 行的 35.62 ms，因为 kernel 从
带宽受限变成 ALU 受限）。所以只有当一次宽 sweep 能替掉**两次**窄 sweep 时才值得。
`encode_rows` 里的循环按 `ceil(rem/8)·55.34 < ceil(rem/4)·35.62` 逐次决策，
而不是查表。`QW_T28_MIN_OUT_F`（默认 256）把宽 kernel 挡在这个模型唯一的小
`out_f`（48，GDN 的 a/b 投影，96/497 个 linear，占权重 0.08%）之外 ——
NR=2 会让它的 grid 从 48 个 threadgroup 掉到 24 个，在 40 个 SM 上不到一波。

### 六、交付二：MPP 的 tile 替换是空操作，出货三轮

`mpp_src_uncached` 会把 `mpp_tiles()` 的值写进 MPP 源码的四个 `#define`。
**其中三个 pattern 写的是源码里早已不存在的值**，所以什么都没替换：

| pattern | 源码里是 | 结果 |
|---|---|---|
| `#define Q4_MPP_KT  32` | `KT  64` | 空操作 |
| `#define Q4_MPP_NRA 64` | `NRA 32` | 空操作 |
| `#define Q4_MPP_NRB 128` | `NRB 256` | 空操作 |
| `#define Q4_MPP_NT  128` | `NT  128` | 命中 |

只有 `NT` 真正生效过。`q4_mpp_mm` 是唯一读这四个宏的 kernel，而它的
**grid 来自 `mpp_tiles()`、token 偏移来自编译进去的值**。所以不一致不是「慢」，
是「只写了一部分输出」。

`mpp_tiles()` 返回 512 而 kernel 里编译的是 256：一个 prefill pass 会开
`rows.div_ceil(512)` 个 M-block，而 kernel 只写 `tgid.x * 256` 开始的 token ——
**前 512 行，仅此而已**。默认 prefill chunk 是 1020 行，所以**任何超过 512 token
的 prompt 都是用「尾巴从没被投影过」的 prefill 答出来的**。

1798 token 的 prompt，重复测量：

| 配置 | 答案开头 |
|---|---|
| NRB=256 | `The user has provided 40 numbered sentences (0 through 39)...` |
| NRB=512（当时的默认） | `The user has sent a message that contains a series of numbered statements (0 through 34)...` |

默认那组连 prompt 里的编号都数错了，因为它从来没看见最后 11 条。

**所有 gate 都没看见它**，每一条都有明确的理由：

| gate | 为什么看不见 |
|---|---|
| 六例 oracle | prompt 5–20 token |
| position pin | 62 token |
| `gemm-check` | 默认 40 token |
| prefill width gate | 比的是 chunk 4 vs chunk 32 |
| `batch-check` | 独立跑走单行 kernel |

**套件里没有任何东西跨过 512 行。**

同一根因还有两件事被一起修正：

1. round 183 把 `QW_MPP_NRB` 从 256 改成 512、记为 1.328× 的 prefill 提速，
   实际是 kernel 少做了一半的活。修好替换之后诚实测量（1020 token，497 linear，
   gemm-bench，3 次，冷却）：

   | NRB | 时间 | TFLOPS |
   |---|---|---|
   | **256** | **1.1445 / 1.1509 s** | **45.7 / 45.4** |
   | 384 | 1.9984 s | 26.2 |
   | 512 | 3.1496 / 3.3311 s | 16.6 / 15.7 |
   | 1024 | 1.5640 s | 33.4 |

   cooperative tensor 每线程持有 `NRB*NRA/NT` 个 float，512 就是 128 个累加器
   寄存器，会溢出；256 是 64，放得下。默认改回 256。

2. 「`QW_MPP_NRA=128` 报 120 TFLOPS 但只写四分之一输出行，所以 gemm-check
   失败」是一条**错误的否定**。kernel 一直用 NRA=32，而 grid 用的是请求值，
   于是恰好写了 32/128 的行 —— kernel 从来没错，是旋钮死的。实测覆盖率对 NRA：
   64 写一半，128 写四分之一，256 写八分之一。现在 NRA 是真旋钮了，
   而 NRA=64/NRB=128 在 1020 token 上是 1.2890 s 对 1.1445 s，
   所以 32 是靠实测留下来，不再是「kernel 只能取这个值」。

`mpp_src_uncached` 现在会**断言每个替换真的发生了**，对不上就 panic 并报出宏名 ——
这是这个 bug 能活过三轮的唯一原因。

### 七、新 gate，以及它必须先证明能抓住这个 bug

`tools/accept.sh` 新增第 22 条 **wide prefill pass**：一个约 700 token 的 prompt，
用 256 行 chunk 和 1020 行 chunk 各答一次，必须一致。期望值故意不存成固定字符串 ——
被测的性质是「两个宽度一致」，这在任何合法的 kernel 改动下都成立。

按「每个新 gate 都要证明能抓住它针对的 bug」的规矩，用**修复前的代码**构建了一个
binary 来跑：**21 passed / 1 failed**（正是这一条 FAIL）；修复后 **22/0**。

第一次跑的时候它在 buggy binary 上**报了 PASS**。原因是 `pkill -f "qwen38 serve"`
匹配的是进程 argv，**匹配不到任何放在 /tmp 里的 A/B binary**；下一个 `serve_up`
于是连上了那个**旧 server**，两个「不同配置」返回同一个答案。现在整个脚本改成
按**端口**杀进程（`stop_serve()`），这个漏法不可能再发生。

### 八、诚实的代价：冷 TTFT 从 1047 变成 ~1250 ms

修好之后 prefill 才第一次做完整的工作，所以冷 TTFT 变慢是必然的。

同 prompt、交替顺序、双方都测「到第一个有内容的 token 的墙钟」（llama.cpp 自己的
`prompt eval time` 不含第一个 token 的采样，拿它跟我们的整个 TTFT 比会美化我们）：

| build | 冷 TTFT（配对交替，3 轮） | vs llama.cpp |
|---|---|---|
| 修复前（跳过 30% prefill） | min 1038.1 / median 1047.1 ms | 1.03× |
| 修复后（诚实） | min 1234.6 / median 1251.1 ms | **1.23–1.32×** |

llama.cpp 那边同机同轮的读数是 933–1030 ms（它自己的 `prompt eval time` 报
1011–1036 ms / 577–585 token）。三次配对的比例是 1.34 / 1.29 / 1.47。

**所以「冷 TTFT 1.029× llama.cpp」这个结论是在一个跳过部分 prefill 的 build 上
得出的，不成立。** 诚实的数字是约 1.3×。

拆开 598 行 prompt 的一次冷请求（1336 ms 那次，`QW_STEP_TIME` + `QW_PREFIX_TIME`）：
...
| 128 | 64 | 0.8600 | 35.6 |
| 64 | 64 | 0.9557 | 32.1 |
| 32 | 256（重复） | 0.8490 | 36.1 |

op 数量（正比于 `ceil(rows/NRB)/NRA`）和单 op 成本正好互相抵消，所以调 tile
拿不到那 28% 的尾部浪费。

### 九、顺手挖出来的 1141 MB

`QW_PREFIX_TIME` 之前给两个 import 和阶梯 export 计时，**唯独没有给每个请求都要付的
prompt-end export 计时**，所以这个数字一直没人看见。加上之后：

```
prefix time: prompt-end export 585 tok 1141 MB 114.477208ms
```
...
**卷积窗口占了 88%，而它 1024 行里只有最后 3 行还会被读** ——
`slot(p) = p % conv_ring`，卷积只回看三行。存 3 行而不是 1024 行，export 会从
114 ms 变成约 1 ms，blob 从 1141 MB 变成约 138 MB，同时每槽 scratch 少 1 GB
（那正是 `batch=64` 需要 112 GB 的原因）。

这是一次 blob 格式变更，需要新 magic + 独立的 gate，所以留给下一轮，没有塞进这一轮。

本轮还顺手去掉了 `blob.clone()`：磁盘存储只借用 blob，而 boundary cache 要拿走所有权，
把磁盘那步挪到前面就能把 1141 MB 的第二次拷贝省掉（约 45 ms）。`accept.sh` 22/22。

### 十、本轮结论

**交付**：

1. 二维分块 kernel `q4_gemv_t28`，宽解码 pass **1.30×**，
   16 路并发聚合吞吐 **69.1 → 89.5 tok/s**。`accept.sh` 22/22。
2. MPP tile 替换空操作的正确性修复 —— 修掉「超过 512 token 的 prompt 用未投影
   的 prefill 回答」，加第 22 条 gate 并证明它能抓住这个 bug，
   把 server 停止改成按端口。
3. 修正「聚合吞吐随并发下降」这个错误结论。
4. 找到并记录 1141 MB / 114 ms 的 prompt-end snapshot，去掉其中一次全量拷贝。

**没做到**：

* otps 仍是 **87–89.5 tok/s**（16 路），对 Splash 的 170。
  4 路并发是 65.2，而 4 路用不到宽 kernel（只有 4 行）。
* 冷 TTFT 诚实的数字是约 **1.25–1.32× llama.cpp**，不达标。
  之前记的 1.029× 是在跳过部分 prefill 的 build 上测的。

**下一步的方向**（按预期收益排序）：

1. **prefix blob 只存卷积窗口的最后 3 行**：114 ms + 1 GB/槽，收益确定、路径清楚，
   只需要新的 blob magic 和一条「resume 与冷跑逐字节相同」的 gate
   （`QW_PREFIX_DUMP` 已经能直接回答这个问题）。
2. **把 4-bit 权重直接喂给 tensor op。** 现在 `q4_mpp_mm` 每个 K-tile 要把
   NRA×KT 个 nibble 展开成 half 再放进 threadgroup memory；MPP 本身有
   `uint4b_format` 操作数。展开这一步（每线程每 op 约 70 条指令）很可能就是
   「45.7 TFLOPS 而不是更高」的原因。
3. **消除尾部浪费**：598 行算 768 行。
...
机器一直没能凉下来（`prefill-bench --tokens 598` 的 best-of-4 是 2247 ms，
冷态是 1081 ms，2.1× 降频；负载均值已经掉到 1.37，所以是热不是别的进程）。
在这个状态下配对交替跑 ours vs llama.cpp：

| build | 冷 TTFT（3 轮） |
|---|---|
| ours | min 2651.4 / median 2724.4 ms |
| llama.cpp | min 2236.7 / median 2325.4 ms |
| **比值** | **1.172×** |

这是目前测到的最好的一个比值（§72ff 的 1.23–1.32× 是在不同降频程度下测的，
所以严格说这些数字之间不可比）。llama.cpp 自己也被同样降频（2237 vs 冷态约 1000），
所以这一轮的**比值**是可用的，**绝对值**不是。

按 §72fh 第四节推算的冷态值（约 1032 ms）仍然是**推算**，要等机器凉下来验证。

## §72fi 解码引擎不是问题，t28 是一个很尖的局部最优

这一轮的目标是 otps。先做了一件从来没做过的事：**把 CONC=4 的墙钟时间拆开**，
看那 42% 的差距到底在哪。

### 一、引擎循环是干净的：96.7% 的墙钟在 pass 里面

`QW_STEP_TIME` 打的 `started at / ended at` 是相对请求时钟的，所以一次运行自己的日志
就能同时算出「pass 时间」和「pass 之间的间隙」。CONC=4，GEN=96：

```
CONC=4 GEN=96  wall 16.28 s  tokens 384  aggregate 23.6 tok/s
  passes 98   in-pass total 15742 ms = 96.7% of wall
  prefill passes 1 (468 ms)   decode passes 97 (15274 ms)
  decode rows histogram: {3: 1, 4: 94, 13: 1, 16: 1}
  inter-pass gap ms: min 2 median 4.0 p90 4 max 81   total 514 ms = 3.2% of wall
  UNACCOUNTED (wall - in-pass - gaps): 19 ms
```

**97 个解码 pass 里 94 个是 4 行**，批处理是好的；间隙中位数 4 ms，只占 3.2%；
没解释的部分 19 ms。所以「65.2 tok/s 对 112 tok/s 的 42% 差距」**不在引擎里，
全在 pass 里** —— 而且这台机器当时 4.2× 降频（4 行 pass 149.8 ms，冷态 35.62 ms），
那个 42% 基本就是降频本身。

### 二、把 §72ff 下一步清单里的两条否掉了

§72ff 列的方向里，第 2 条（4-bit 直接喂 tensor op）和第 4 条（降低解包 ALU）
都是关于「解包 ALU 是墙」。这一轮做了三个隔离测量，**都在同一个进程里轮转**
（`bench --rows 21`，首臂在末尾重复做仪器标定）：

**(1) 解包不是吞吐瓶颈。** 造了一个诊断 kernel `q4_gemv_t28_diag`，和 t28 一模一样，
只把每个 packed word 的**高半部分复用低半部分的解包结果** —— 去掉每行每轮 4 个
shift+and+cvt 和 4 个 scale/bias 应用，**所有 load 和所有 dot 一个不少**。
8 token：

| arm | ms | 校准后 vs base |
|---|---|---|
| t28 | 144.06 | 0.5919 |
| t28-DIAG（少一半解包，结果错） | 147.35 | 0.6054 |

**少了 11% 的解包指令，反而慢了 2.3%**（标定偏差是后跑的臂快 15%，所以真实差距更大）。
如果解包是吞吐瓶颈，这里应该快 11%。它慢下来是因为 `w1[r] = w0[r]` 把两条独立的
解包链变成了一条依赖链 —— 也就是说 **t28 是延迟/ILP 受限，不是解包吞吐受限**。

**(2) NK 越大越好，但 16 个累加器就掉下悬崖。** 解包是**每个输出行付一次、按 NK 摊薄**，
所以 ops/MAC 随 NK 下降：NK=8 → 1.23，NK=12 → 1.05，NK=16 → 0.96。16 token：

| arm | ms | vs base | 校准后 |
|---|---|---|---|
| k4-flat ×4（今天的引擎） | 480.85 | 1.0000 | 0.9842 |
| t28 (NK8 NR2) ×2 | **311.99** | **0.6488** | **0.6385** |
| t212+t24 (NK12+4) | 426.75 | 0.8875 | 0.8734 |
| t216 (NK16 NR2) ×1 | 481.13 | 1.0006 | 0.9847 |
| t48 (NK8 NR4) ×2 | 493.67 | 1.0267 | 1.0104 |
| t28w (NK8 NR2, 16B 权重 load) ×2 | 374.76 | 0.7794 | 0.7670 |

NK=12（24 个累加器）比 NK=8 慢 **1.37×**，NK=16（32 个）慢 **1.51×**。
所以「ops/MAC 更低」完全被寄存器溢出吃掉了，**16 个累加器就是那道坎**。

**(3) 16 字节权重 load 也是更慢。** k4-flat 用 16 字节权重 load 能跑到 ~7.9 TB/s 的
load 流量，而 t28 只有 ~5.1 TB/s —— 而且 t28 每个 (token × 输出行) 搬的字节**更少**
（8.5 对 17.0）。搬得更少却更慢，说明不是带宽受限而是延迟受限，所以照 k4-flat 的做法
给 t28 也换成 16 字节权重 load（`q4_gemv_t28w`，一个 lane 一次拿 4 个 word，
且这 4 个 word 一定在同一个 64 权重 group 里，因为 `4*lane mod 8 ∈ {0,4}`）：

| arm | 8 token ms | vs base | 校准后 |
|---|---|---|---|
| t28 | 144.06 | 0.5035 | 0.5919 |
| t28w（16B 权重 load） | 236.82 | 0.8277 | 0.9731 |

**慢 1.64×**。ops/MAC 其实更低（1.03 对 1.23），但每个 lane 要同时持有
`half4 wv[NR][8]` = 16 个 half4 = 32 个寄存器，加上 16 个 float 累加器，直接溢出。

### 三、结论：t28 是一个很尖的局部最优

在同一台机器、同一个进程、同一轮里，从 t28 往任何一个方向走都更差：

| 方向 | 相对 t28 |
|---|---|
| NR 4→2（t44/t48） | 1.30–1.58× 慢 |
| NK 8→12 | 1.37× 慢 |
| NK 8→16 | 1.51× 慢 |
| 权重 load 4B→16B | 1.64× 慢 |
| 去掉一半解包 | 1.02× 慢 |

**所以「降低解包 ALU」这条路（§72ff 第 4 条）和解码路径上的「喂 tensor op」
（第 2 条）都应该从清单上划掉。** 解码 pass 的墙是真实的，而这个 kernel 家族里
没有更好的形状。

### 四、那 otps 的墙到底在哪 —— 算术

权重每个 pass 读一次：14.412 GB ÷ 477.9 GB/s = **30.16 ms/pass**，与行数无关。

* CONC=4（4 行/pass，k4-flat）：上限 4 / 0.03016 = **132.6 tok/s**，实测 4 行 pass
  35.62 ms → 112 tok/s（上限的 85%）。
* CONC=16（16 行/pass = 2×t28）：pass 被 ALU 墙卡在 6.9 ms/token → 107.8 ms →
  **148.4 tok/s**。

**这就是两个上限：4 路 132.6（DRAM 限），16 路 148.4（ALU 限）。** 要越过 148.4
必须把 6.9 ms/token 降下来，而这一轮证明了改 kernel 形状降不下来。

**关于 Splash 的 170：** 它是 M5 Pro（带宽约 273 GB/s，比 M5 Max 的 478 低）。
4 路并发要 170 tok/s 就是 4 token / 23.5 ms 一个 pass，而 14.412 GB 在 273 GB/s 下
就要 52.8 ms。**这个数字在 M5 Pro 上按「每个 pass 读一遍全部权重」的模型是算不出来的**
（即便 M5 Max 也要 30.16 ms，上限 132.6）。要么他们的 pass 不读全部权重，要么
170 不是在 4 路并发下按这个口径测的。**这一点值得记下来，但我不打算用它当借口** ——
我们的 16 路上限 148.4 对 170 仍然是实打实的差距。

### 五、本轮交付

**做到的**：

1. 解码墙钟的完整分解：**96.7% 在 pass 内，间隙中位 4 ms，未解释 19 ms** ——
   引擎循环没有可捡的东西，otps 只能从 pass 时间里来。
2. 否掉两条方向，每条都有同进程轮转的隔离测量：解包不是吞吐瓶颈（去掉一半反而慢 2.3%）、
   NK=12/16 与 16 字节权重 load 都是溢出（1.37×/1.51×/1.64×）。
3. 确认 **t28 是尖锐局部最优**，16 个累加器是那道坎。
4. 新增可复现的 bench 臂：`t212`、`t216`、`t28w`、`t28-DIAG`（诊断，结果错）。

**没做到**：

* otps 一点没涨。这一轮是**排除法**：把 otps 的两条候选路走死，换来「墙在别处」的结论。
* 冷/热 TTFT 本轮没测。

**下一轮**：既然 kernel 形状走到头了，剩下的是**架构**层面 ——
唯一的候选是 MPP tensor path 在小行数下的 253–370 ms 固定开销（1020 行只要 1145 ms，
...
## §72fm batch 64 走到底：更差；以及 pass 级基准骗了我三轮

### 一、把 §72fk / §72fl 指出的那条路走完

§72fk 定位 batch 64 是下一个 otps 杠杆，§72fl 算出它被内存挡住（模型 165.7 GB、
按实测比例推算 142 GB，都超 128 GB），并指出唯一**不牺牲冷 TTFT** 的出路：
`snap` 每 slot 576 MB，只在 `decoding == 1`（spec verify）时用，可以只留一份。

这一轮把它做了：

* `snap` 从 `hv*dv*dk*4*TILE*batch` 改成 1 个 slot；
* `snap_stride` 保持「每序列」的值，所以**内核侧的所有偏移一字未改**；
* `spec_slot` 钉回 slot 0（engine.rs 原注释写着它被**特意**从 slot 0 泛化过，
  我改回去并写明了代价：落在别的 slot 上的孤立请求不再 spec）；
* `budget()` 把 snap 从 per_slot 移到固定项。

结果：batch 64 的模型需求 161 → **126 GB**。`check_memory` 仍拒绝（它要留余量，
自己的建议是 batch 64 配 `--max-ctx 4014`），但 **batch 48 在完整 8192 context 下能起**
（94 GB，占机器 73%）。

### 二、48 实测更差，而且 pass 级模型是错的

pass 级（`prefill-bench --batch 1`，一个序列里放 N 行）：
48 行 NRB=64 = 0.1499 s，32 行 NRB=32 = 0.1193 s → 320 对 268 tok/s，预测 1.19×。

服务实测（`conc4.py`，CGEN=96，交替）：

| | CONC=32 | CONC=48 |
|---|---|---|
| 第一对 | 111.4 / 81.2 | 168.6 / 159.7 |
| 第二对 | 88.8 / 107.2 | 102.1 / 73.7 |

**CONC=48 更差。** 引擎直方图确认它确实在跑 48 行（94 趟正好 48 行），
但一趟约 336 ms，而 32 行约 154 ms —— **1.5× 的 token 要付 2.2× 的时间。**

**原因：pass 级 benchmark 是「一个序列里放 N 行」，而服务里的一趟是「N 个序列各 1 行」。**
后者把每一个 GDN state 写和 KV 写都打散到不同的 slot 步长上。
**§72fk 和 §72fl 里所有 `prefill-bench` 的 pass 宽度数字都高估了服务吞吐**，
因为内存模式根本不同。这一条直接修正了我前两轮「并发度就是 otps」的判断。

另外，**更大的分配本身就是成本**（统一内存）：同样 CONC=32，
`QW_BATCH=48` 反而比 `QW_BATCH=32` 差（73.7 对 109.6）。

### 三、snap 改动本身没有可验证的收益，已回滚

* **RSS 完全没变**：batch 32 下新旧都是 81.1 GB。和 §72fl 删死缓冲时一样——
  没被写过的分配不会变成驻留页，省下的是模型里的账，不是内存。
* **otps 在噪声里**：batch 32 对齐后，前两轮新胜（102.6 / 109.6 对 88.8 / 107.2），
  后三轮旧胜（5.4 / 5.0 / 4.4 对 5.3 / 4.8 / 4.0）——但后三轮整机已经崩到 **4–5 tok/s**
  （从 100–185 掉下来，约 25× 降频），那些数字没有意义。
* **代价是真实的**：spec 被限制在 slot 0。

**回滚。** 一个没有可验证收益、却带来行为限制的改动不该留下。
`crates/` 现在和 `53baf5c` 逐字节相同，而那个状态已经通过 `accept.sh` 22/22。

### 四、本轮结论

**交付**：

1. 把 §72fk 指出的 otps 路径走到底并**否掉**：batch 48/64 在真实服务下更差，
   证据是交替实测 + 引擎 pass 直方图（94/94 趟正好 48 行，排除「没跑满」的解释）。
2. **发现 pass 级基准与服务的内存模式不同**（1 序列 × N 行 对 N 序列 × 1 行），
   这解释了并发度为什么没有兑现，并修正了我自己前两轮的推断。
3. 验证了 `snap` 单 slot 化对 RSS 和 otps 都没有可测影响，据此回滚。
4. 交付物是一份否证 + 一条方法论修正，不是性能数字。

**没做到**：

* **otps 没有推进。** 最好成绩仍是 §72fk 的 151.1 tok/s（对 Inco Splash 的 170，89%）。
* 冷 / 热 TTFT 本轮没测。
* 机器在连续测量后崩到 4–5 tok/s，需要冷却才能再取干净数据。

**下一步**：`prefill-bench` 必须能按「N 个序列各 1 行」的方式测（即真正的
batch 解码形状），否则任何关于 pass 宽度和并发度的判断都会重复这个错误。
这是后面取干净数据的前提。

## §72fn 冷 TTFT 实测 1.563×，和一个 1.32–1.67× 的预填充修复

### 一、冷 TTFT 第一次在冷机上干净测量：1.563×，不是 1.172×

§72fk 记的 1.172× 是在降频机上测的——两边都被压扁，低估了差距。这一轮机器完全冷却
（32 行 pass 0.1094–0.1115 s，冷机基准 0.1193 s），用 `ttft_paired.py` 配对测量：
交替顺序、每轮换新 prompt（两边都缓存前缀）、两边都按「请求到第一个带内容 token 的墙钟」计。

| | 我们 | llama.cpp |
|---|---|---|
| runs | 1704 / 1770 / 2084 / 1803 | 1464 / 997 / 1276 / 1010 |
| median | **1786.5 ms** | **1142.6 ms** |

**ours/llama = 1.563×。冷 TTFT 没有达标，差 56%。**

### 二、分解：预填充就是全部

`QW_STEP_TIME` 加上这一轮新加的 `QW_SPLIT_TIME`：

```
prepare:  total 3.5 ms
split:    581 rows, set_tokens 5.2 ms, forward_rows 1380.7 ms
step time: 581 rows, took 1.3807 s
step time: 4 rows,  took 40.0 ms
prefix time: prompt-end export 183 MB 21.5 ms
```

预填充占 1381 / 1460 = **95%**；其中 MPP GEMM 又占 85%（581 行：gemm-bench 0.816 s，
prefill-bench 0.959 s）。

**参数层面已经到顶**（581 行，gemm-bench）：NRA=16 慢 1.15×，NRA=16/NRB=512 慢 1.73×，
NRA=32/NRB=512 慢 3.6×，NRA=64/NRB=512 慢 14×，KT=128 持平，NRB=128 快 1.05×。
`set_tokens`（主机侧 embedding 反量化）只有 5.2 ms，不是瓶颈。

### 三、修掉一个 1.32–1.67×：MPP tile 必须按行数选，不能固定

一趟 pass 是按整块 tile 计费的，所以 581 行用 NRB=256 是 3 块、284 行是 2 块
（容量 512，**浪费 45%**）。跨全部 497 个 linear 拟合：

```
time ≈ 25.4 ms × ceil(rows/nrb) + 0.988 ms × ceil(rows/nrb) × nrb
```

（300/581/900/1020 四点拟合，最大误差 3%：300 行预测 443.4 ms，实测 443.4 ms。）

**只按容量挑是错的**：900 行时 NRB=64 浪费最少（960 对 1024），却仍然慢 1.09×——
15 块比它省下的 64 行更贵。所以要最小化拟合代价。

`mpp_mid_wins()` 用这个模型在 mid(64) 和 wide(256) 之间选，并且**在 `rows <= 256` 时
不查模型**：那时 wide 是单块，实测必胜，而模型在 140 行会预测错 1.17×（三个 64 宽块
比拟合假设的更贵）。模型在 300/320/340/400/520/560 六点与 gemm-bench **全部一致**。

端到端（284 行 prompt，交替两轮）：

| | 旧路由（wide） | 新 chooser |
|---|---|---|
| 冷 TTFT | 1323.3 / 1320.1 ms | 988.8 / 796.3 ms |
| prefill step | 1.2203 / 1.2198 s | 0.9257 / 0.7319 s |

**预填充快 1.32–1.67×，284 行 prompt 的冷 TTFT 从 1320 ms 降到约 890 ms。**
（`old` 两次重复到 0.2%，所以这不是漂移。）

`accept.sh` **22 passed / 0 failed**。路由改动曾经在 §72fk 挂过 5 个门，所以这次先跑门再取数。

### 四、还剩下一个没解释的 300 ms「首个请求税」

同一台服务器上连发 3 个请求（改过 prompt，避开前缀命中）：

```
req0  581 rows  forward_rows 1.1736 s   TTFT 1262.6 ms
req1  583 rows  forward_rows 0.9616 s   TTFT 1042.6 ms
req2  584 rows  forward_rows 0.9812 s   TTFT 1068.0 ms
```

**启动后第一个请求的 forward_rows 比稳态慢约 300 ms。** 而 `ttft_paired.py` 正是
「每次重启服务器、只发一个请求」，所以这 300 ms 被**全额计入被测的冷 TTFT**。

**加载期 warmup 吸收不掉它**：把 warmup 行数从 64 提到 160、再到 1020（一趟 1020 行
要 1.5 s，加载时间 4.8 s → 7.1 s），首个请求都还是 1.37 s。
（顺带否掉了「wide tile 是懒编译」这个假设——它本来也没被 warmup 覆盖，因为
§72fk 之后 tile 会按行数选，而 warmup 只有 64 行。）但**先发一个别的请求就能吸收**。

**这是目前最大的单条线索：约占被测冷 TTFT 的 20%，且已经知道它是「先来一个请求就消失」
的进程内一次性开销，不是数据相关的。**
...
1. 冷 TTFT 从 §72fk 的 1.172×（降频机，低估）修正为 **1.563×**（冷机配对实测）——
   这是本项目第一次拿到可信的冷机冷 TTFT 数字。
2. 一个**已验证的 1.32–1.67× 预填充修复**：284 行 prompt 的冷 TTFT 1320 → 890 ms，
   由拟合代价模型驱动 tile 选择，模型在六个宽度上与实测一致。
3. `accept.sh` 22/22。
4. 新增两个诊断开关：`QW_SPLIT_TIME`（拆分 set_tokens / forward_rows）、
   `QW_WARM_ROWS`（warmup 行数）。

**仍未达标**：

* **冷 TTFT 1.563×**，差 56%。注意这个修复只覆盖 257–384 行的 prompt；
  配对基准用的 581 行 prompt 仍走 wide tile，所以 **1.563× 还没被这一轮改善**。
* **otps 未动**，最好仍是 §72fk 的 151.1 tok/s（对 170，89%）。
* 热 TTFT 仍然有效达标（prompt-end blob 直接恢复，7.8 ms，逐字节一致）。

**下一步**：查那 300 ms「首个请求税」。已知它是进程内一次性开销、能被一个前置请求吸收、
且加载期 warmup 吸收不掉——这三点把它限定在「第一次真的按请求路径跑」的那部分代码里
（前缀缓存的首次走查、第一份 183 MB 快照的导出、或第一个请求触发的某种惰性分配）。
按被测冷 TTFT 的 20% 算，这是目前性价比最高的一条。

## §72fo 冷 TTFT 复测 1.405×；prefill-bench 不能代表服务端单趟代价

### 一、复测：1.405×（上一轮记的是 1.563×）

同一个 `ttft_paired.py`，机器在冷态（32 行 pass 0.1198 s = 冷机基准）：

| | 我们 | llama.cpp |
|---|---|---|
| runs | 1498 / 1386 / 1253 / 1433 | 1003 / 992 / 1004 / 1005 |
| median | **1409.8 ms** | **1003.6 ms** |

**ours/llama = 1.405×。**

两点比数字本身更重要：

1. **llama.cpp 的重复性是 ±1.3%（991.8–1004.9 ms），我们是 ±20%（1253–1498 ms）。**
   同样的 prompt、同样的交替顺序，我们的方差是对方的 15 倍。
2. 1.563× → 1.405× **不是代码带来的**：配对 prompt 是 581 token，正好落在 §72fn 那个
   chooser 不覆盖的区间（>384 行走 wide tile），所以这一轮没有改动它。
   这是机器状态差异，不能记成优化。

### 二、prefill-bench 不是一个有效的「服务端单趟」模型——两个独立原因

§72fm 发现了第一个：它的 pass 是「1 个序列 × N 行」，而服务端的一趟是「N 个序列 × 1 行」，
内存模式不同。

这一轮发现第二个，而且更根本：**它的 reset 不计入计时，服务端的 reset 计入。**

```rust
// prefill_bench
models[i].reset();                 // 主机侧 zero()，约 1.1 GB memset，在计时器之外
let t = Instant::now();
models[i].forward_rows(&rows)?;    // 只计这一个
```

`Qwen38::reset()` 是主机侧 `zero()`——逐层 `zero(&g.state)`、`zero(&g.window)`、
`zero(&a.k_cache)`、`zero(&a.v_cache)`。而服务端走 `reset_seq_async()`：一个 GPU kernel
（`zero_half`），故意用 `finish(wait=false)` 排队，代价落进随后 `forward_rows` 的阻塞等待里。

**所以 §72fn 那个「服务端比 bench 慢 1.41×」的结论，至少有一部分是两个计时器包含的
东西不同造成的，不是服务端真的慢 41%。** 那 3 轮交替测量里差异确实存在（0.968 对 1.36），
但归因错了。
...
| 假设 | 结果 |
|---|---|
| spec 解码 / MTP 头 | **否**。`QW_SPEC=0` 与默认相同（1.365/1.360 对 1.175/1.380） |
| max-ctx（服务端 8192 对 bench 的 1020） | **否**。8192 → 1.386/1.174，1024 → 1.326/1.323，完全重叠 |
| slot 分配（batch 1 对 32） | **否**（§72fn 已测：1%） |
| warmup 行数 64 / 160 / 1020 | **否**。首个请求三档都是 1.37 s |
| NRB=320（模型预测 2 块覆盖 581 行，1.22×） | **否，反而更慢**：0.9631 对 0.8151 |
| NRB=384 | **否，更慢**：1.3108 |
| keep-warm 能消掉它 | **只值约 40 ms**：1.368/1.381（=30）对 1.417/1.408（=0） |

**成本模型在 NRB ≥ 320 处失效**：`25.4 ms/块` 这个常数项会随 NRB 上升（寄存器压力）。
所以「用 2 块代替 3 块」这类推理不能再沿用拟合模型。
（581 行实测：NRB=128 → 0.7775，192 → 0.8552，256 → 0.8151，320 → 0.9631，384 → 1.3108。
最好仍是 128，比 256 快 1.05×。）

### 四、顺带发现：LM head 对所有行都算

`forward_rows` 里 `let n = rows.len();`，最后

```rust
lm_head.encode_rows(..., n);
```

LM head 是 `248320 × 5120`——全模型最大的一个量化线性（约 0.72 GB）。581 行 → NRB=256
→ **3 块**，tile 工作量 `2 × 768 × 248320 × 5120 ≈ 1.95 TFLOP`，按 §72fj 测到的
47 TFLOPS 约 **41 ms**；而纯 prefill 只需要**最后一行**的 logits（1 行 → t28 GEMV，约 2 ms），
logits 写回也从 288 MB 降到 0.5 MB。

**约 40 ms，占冷 TTFT 3%。** 但同一条路径也被 spec verify 用（那里确实需要多行 logits），
所以必须条件化，不能无条件改——本轮没有动它。

### 五、结论

**交付**：

1. 冷 TTFT 复测 **1.405×**，并指出 llama 的稳定性是我们的 15 倍。
2. 证明 `prefill-bench` 不能代表服务端单趟代价，**第二个原因：主机侧 reset 不计时**。
   这是本轮最有价值的产出——它说明前面几轮反复出现的「服务端比 bench 慢 1.4×」
   被误归因了，后续不该再拿 bench 当服务端的代理。
3. 逐条否掉 6 个候选机制（spec、max-ctx、batch、warmup 宽度、NRB=320、NRB=384），
   并发现拟合成本模型在 NRB ≥ 320 失效。
4. 量化 keep-warm 只值约 40 ms（注释里暗示的是约 270 ms）。
5. 发现 LM head 全行计算（约 40 ms 可回收，未动）。
...
1. **在服务端单趟内部加细粒度计时**（reset / MPP GEMM / 尾部小 kernel / head 分开）。
   黑盒 A/B 已经把候选机制穷举完了，而那 400 ms 仍未定位——这是唯一还没试过的定位手段，
   而且 `QW_SPLIT_TIME` 证明这种办法有效（它一上来就把 `set_tokens` 排除掉了）。
2. **LM head 在 prefill 时只算最后一行**（约 40 ms，需保证 spec verify 路径逐字节不变）。
3. 给 513–640 行加 NRB=128 变体（实测 1.05×，需第 4 个 kernel 和一次完整 accept.sh）。
