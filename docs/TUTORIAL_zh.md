...
* **并发 = 1**：模型跑在一个专用线程上，请求经 `mpsc` 排队串行处理。
  ...
  **实测**（3 个各 18 token 的请求）：

  | | 第 1 个 | 第 2 个 | 第 3 个 | 总墙钟 |
  |---|---|---|---|---|
  | 顺序发起 | 2.43 s | 5.08 s | 7.76 s | 7.76 s |
  | 同时发起 | 2.73 s | 5.38 s | 7.96 s | 7.96 s |

  同时发起时结束时间呈**规则阶梯**（2.73 / 5.38 / 7.96），总墙钟比 1.03 —— 即完全串行，
  并发不会提升总吞吐。**再加进程也没用**：两个进程各服务一个请求需 4.18 / 4.60 s，
  而单进程服务两个请求是 2.21 / 4.53 s，**加速比 0.99×**。
  原因是这个引擎受**内存带宽**限制：每个请求都在流式读取同一份 14.4 GB 权重，
  两个进程只是把这同一份带宽对半分。**结论：想靠多开进程扩展吞吐是徒劳的，还会白吃内存（每进程 15 GB）。**
...
> **并行/batch-16 进行中**：目标是把引擎改成支持最大 batch 16 的并发推理。
> 现状（第 2 轮）：**使能内核已经做出来了**——`q4_gemv_b16` 让 16 个连续 threadgroup
> 共用同一权重行（DRAM 读一次、其余命中 L2），数值正确（8 个张量 norm_err 0.27–1.39%），
> 在全模型 14.4 GB 上**比串行快 1.666×（配对校准）**，16 路并发聚合 **52.2 tok/s**；
> 而第一版"每线程扛 16 行"的内核被证伪（0.483×）。
> **批量前向已经打通（第 4 轮）**：`forward_rows(rows)` 一次前向推进 16 条序列，
> 整块 14.4 GB 权重只读一次。门禁 `qwen38 batch-check` 证明它与 16 次独立前向
> **逐位等价**（worst |logit diff| = 0.0000），因此批量不会改变任何输出。
> **但引擎还没接上**：**每序列状态分区已经做完并锁住了**——用字节偏移把 KV / GDN / conv
> 状态按 16 条序列切开，内核一行没改；门禁是 `QW_BATCH=16 QW_SLOT={0,7,15}` 跑同一 prompt
> 必须与单 slot 基线**逐字节相同**（已通过），且 `verify --oracle` 仍 parity 6/6。
> 还差：**服务端调度器**（`Engine` 仍一次一个 Job）、MTP 草稿头的批量化。**所以本教程 §6.4 的"并发 = 1"仍然是当前事实。**
