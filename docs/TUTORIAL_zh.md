...
| 多并发请求 | ✅ 最大 16 路真并发 | 一轮一次批量前向推进所有槽，见 §6.4 |
| 图片/多模态 | ❌ 没有 | 纯文本 |
...
* **并发 = 最大 16 路**：模型跑在一个专用线程上，但**不再排队串行**。最多 16 个请求
  各占一个序列槽（slot），**每一轮一次 `forward_rows` 前向同时推进所有活跃槽各一个 token**。
  关键就在这一句：那一次前向只读一遍 14.4 GB 权重，却服务了整个 batch ——
  这是并发能提升**总吞吐**的唯一原因，也是为什么它需要改内核而不只是加个队列。

  **实测**（16 个互不相同的请求，每个最多 24 token）：

  | | 总墙钟 | 平均每请求 |
  |---|---|---|
  | 16 个同时发起 | **22.02 s** | 1.38 s |
  | 同样 16 个逐个发起 | 73.36 s | 4.59 s |

  **加速比 3.33×**，且 **16/16 个响应与"该请求单独跑"逐字节一致**（门禁 `tools/conc.sh`）。
  达不到 16× 有两个原因：一轮 pass 的代价随 batch 变大而上升（16 行一轮约 306 ms，
  约是单行的 10.2 倍而非 16 倍），且各请求生成长度不同 —— 先结束的槽空出来，
  剩下的仍得一轮一轮推进，尾部利用率自然低。

  **注意"多开进程"这条路已被实测证伪**：两个进程各服务一个请求，**加速比 0.99×**。
  因为每个进程都在流式读同一份 14.4 GB 权重，只是把同一份内存带宽对半分，
  还白吃内存（每进程 15 GB）。**必须在一个进程内共享那次权重读取** —— 这正是 batch 轴的意义。

* **内存：这是最容易踩的坑。** 实测（`ps` 的 RSS）**ctx 8192 约 41 GB，ctx 32768 约 65 GB**
  —— 约 33 GB 固定开销 + **1.0 MB/token**。每个 slot 自带 KV cache
  （**64 KB/token/slot**）、GDN 递推态、卷积环和投机回滚快照，而 16 份是**启动时一次性分配**的，
  所以 **KV 成本同时乘以 `max-ctx` 和 batch 宽度**。
  想要更长上下文就调小 batch：`QW_BATCH=1 --max-ctx 262144`（模型自身上限就是 262144）。
  服务端现在会**按物理内存预估并拒绝放不下的配置**，并在报错里直接给出"这台机器最大能放到多少"。
  在加这个校验之前，`--max-ctx 1000000` 的表现是：进程在分配时被系统 **SIGKILL**，
  没有报错、端口从未绑定，你只能看到 `curl: (7) Failed to connect` —— 极难排查。
* **这条路径上投机解码（MTP）是关闭的**：草稿头目前只有单序列状态，批量路径跑纯解码。
  单请求串行时的投机加速（见 §6.3）在 16 路并发下换成了总吞吐 —— 这是有意的取舍。

### 6.6 prefill 是逐 token 全量扫权重 —— Agent 长对话的真正瓶颈

这一点比并发限制严重得多，**做 Agent 前必须知道**。

引擎把 prompt **一个 token 一个 token** 喂进解码路径（每个 prompt token 都要完整扫一遍
14.4 GB 权重），没有批量 prefill，也没有跨请求的前缀缓存（每个请求开头都 `model.reset()`）。

**实测**（固定生成 4 token，只变 prompt 长度）：

| prompt token | 首 token 前耗时 | 摊到每 prompt token |
|---|---|---|
| 10 | 0.48 s | 47.6 ms |
| 109 | 4.10 s | 37.6 ms |
| 409 | 20.9 s | 51.1 ms |
| 1209 | 78.0 s | 64.6 ms |
| 3009 | **253.8 s（≈4.2 分钟）** | 84.4 ms |

（每 token 成本随测试进行而上升，是机器持续升温导致的，见 §8.2。）

对比：解码一个 token 约 144 ms，但投机解码一次前向出 ~3.4 个 token，
所以**摊到每个 token 上，prefill 比 decode 贵约 3.4 倍**。

**含义**：一个 3000 token 的对话历史，每轮要在**出第一个字之前**先等 4 分钟；
而且因为每个请求都 `model.reset()`，**下一轮要把整个历史重新 prefill 一遍**。
N 轮 Agent 的总 prefill 开销是 O(N × 历史长度) —— **平方级增长**。

> **并行/batch-16 已完成**：引擎现在支持**最大 16 路真并发**。四步分别是：
> * **使能内核**：`q4_gemv_b16` 让 16 个连续 threadgroup 共用同一权重行
>   （DRAM 读一次、其余命中 L2），数值正确，全模型 14.4 GB 上**比串行快 1.666×（配对校准）**；
>   而第一版"每线程扛 16 行"的内核被证伪（0.483×）。
> * **批量前向**：`forward_rows(rows)` 一次前向推进 16 条序列，整块 14.4 GB 权重只读一次。
>   门禁 `qwen38 batch-check` 证明它与 16 次独立前向**逐位等价**（worst |logit diff| = 0.0000），
>   所以批量**不改变任何输出**。
> * **每序列状态**：用字节偏移把 KV / GDN / conv 状态按 16 条序列切开，内核一行没改；
>   `reset_seq(slot)` 只清零一条序列的递推态（KV cache 不用清，它是位置索引的，
>   越过该序列自身位置的表项永远读不到），每槽清理量从 ~700 MB 降到 ~165 MB。
> * **服务端调度**：`Engine` 持有 16 个槽位，每轮一次批量前向推进全部活跃槽。
>   门禁 `tools/conc.sh` 实测 16 并发 **22.02 s vs 串行 73.36 s = 3.33×**，
>   **16/16 响应与单独跑逐字节一致**。**本教程 §6.4 已按此改写。**
> * 仍差：MTP 草稿头的批量化（批量路径跑纯解码）。
> 设计与实测见 **`docs/PLAN_BATCH16.md`**。
>
> **因此的实操建议**：
* 把系统提示、工具定义压到最短；不要塞长文档进上下文；
* Agent 轮数控制在个位数；超过 5–6 轮基本不可用；
* 宁可单轮 RAG（一次长 prompt、一次回答）也不要长 Agent 循环；
* 客户端超时设到 **600 秒以上**。

> **改进方向（已在 `loop/state.json` 立项）**：批量 prefill —— 现成的 verify 内核
> （`q4_gemv_k4_u4hx`）**一次前向已经能算 4 行**，把它用于 prefill 即可让 prompt 每 4 个 token
> 才扫一遍权重，预期 prefill 提速约 4×；进一步的跨请求前缀缓存可把多轮对话从平方级压回线性。
> 但两者都要建立在"批量行与单行结果一致"这个前提上 —— 而那正是 `spec==plain` 偶发失败
> 所质疑的性质，所以那件正确性问题应当先解决。
...
| 并发请求很慢 | 已支持 16 路并发（§6.4）；**超过 16 个会排队** | 并发数控制在 16 以内 |
| `curl: (7) Failed to connect` | 服务端启动时被系统 Kill，多半是 `--max-ctx` 太大 | 看启动输出；现在服务端会直接告诉你能放多大 |
| `max-ctx N does not fit` | 16 个 slot 的 KV 一起分配，超了物理内存 | 按提示调小 `--max-ctx`，或 `QW_BATCH=1` 换长上下文 |
