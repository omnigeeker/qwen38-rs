...
> 第一轮已实测：新增的 16 行 GEMV 内核**数值正确**（8 个张量 norm_err 0.27–1.39%），
> 但在全模型 14.4 GB 上**比串行慢 22%（0.78×）**——权重摊销没有兑现，
> 原因与下一版设计（L2 共享权重）见 **`docs/PLAN_BATCH16.md`**。
> **所以在功能真正落地前，本教程 §6.4 的"并发 = 1"仍然是当前事实。**
>
> **因此的实操建议**：
