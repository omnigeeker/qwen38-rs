...
struct Kernels {
    ...
    q4_gemv_tile: Kernel,
    /// 2-D (features x tokens) flat tile, NR=2 rows by NK=8 tokens per
    ...
    /// The same MPP GEMM compiled with a 32-wide token tile.
    ///
    /// The descriptor's M extent is a compile-time constant, so a threadgroup
    /// always processes NRB tokens even when the pass has fewer.  With NRB=256 a
    /// sixteen-row decode pass cost the same as a 256-row one - measured
    /// 0.4361 s against 0.1047 s at NRB=32 on the real 497 linears.  This is the
    /// pipeline a short pass uses.
    q4_mpp_small: Option<Kernel>,
    /// The MPP GEMM compiled with a 64-wide token tile, for passes of 33..128 rows.
    q4_mpp_mid: Option<Kernel>,
    rmsnorm: Kernel,
    rmsnorm_ws: Kernel,
    rmsnorm_nw: Kernel,
    rmsnorm_tile: Kernel,
    ...
    pub fn load_batch(dir: &Path, max_t: usize, batch: usize) -> Result<Self> {
        ...
        let kernels = {
            ...
            Kernels {
                ...
                // The wide tile: NR=2 output rows by NK=8 tokens per threadgroup.
                // The old occupant of this slot, `K_Q4_GEMV_B16`, was refuted in
                // round 056 and its dispatch never ran (`encode_rows` ignored the
                // argument).  Isolated full-sweep, paired, 5 rounds, 8 tokens either
                // way: the engine's current 2 x k4-flat costs 71.02 ms and this
                // costs 55.34, ratio 0.7792 raw / 0.7888 against a duplicate-arm
                // calibration of 0.9878.  At 16 tokens it is 107.80 against 140.27,
                // 0.7685 / 0.7696 with a 0.9987 calibration.  The kernel source
                // carries the reasoning: the flat kernel's activation reads are
                ...
                q4_mpp_mid: match b.kernel(msl::mpp_src_mid(), "q4_mpp_mm") {
                    Ok(k) => Some(k),
                    Err(e) => {
                        eprintln!("q4_mpp_mm (middle tile) FAILED TO BUILD ({e}); 33..128-row passes fall back to the wide tile");
                        None
                    }
                },
                q4_mpp_small: match b.kernel(msl::mpp_src_small(), "q4_mpp_mm") {
                    Ok(k) => Some(k),
                    Err(e) => {
                        eprintln!("q4_mpp_mm (narrow tile) FAILED TO BUILD ({e}); short passes fall back to the wide tile");
                        None
                    }
                },
            ...
            match &layer.kind {
                Kind::Full(a) => {
                    ...
                    a.q.encode_rows(
                        ...
                        kernels.q4_mpp_small.as_ref(),
                        kernels.q4_mpp_mid.as_ref(),
                        &scratch.h,
                        &scratch.qg,
                        n,
                    );
